// ⚠️ IMMEDIATE DEBUG LOG - This should appear FIRST
console.log('🚀 SCRIPT STARTED - tamagotchi-game.html loaded');
console.log('📍 Script execution beginning at:', new Date().toISOString());
...
console.log('✅ Skins config loaded, enabled skins:', getEnabledSkins().length);
...
// 🐾 CRITICAL: Initialize pet display IMMEDIATELY (before DOMContentLoaded)
...
(function () {
  try {
    ...
    if (document.body && !document.body.getAttribute('data-pet')) {
      ...
      console.log('✅ Early pet initialization - data-pet set to:', savedPet);
      ...
      document.addEventListener('DOMContentLoaded', function () {
        ...
        if (!document.body.getAttribute('data-pet')) {
          document.body.setAttribute('data-pet', savedPet);
          console.log('✅ Early pet initialization (DOM ready) - data-pet set to:', savedPet);
        }
      });
    }
  } catch (e) {
    console.warn('⚠️ Early pet initialization failed:', e);
  }
})();
...
// ⏰ HALVING SYSTEM - Daily Pool Calculation
...
function checkDailyReset() {
  ...
  if (gameState.lastResetDate !== today) {
    console.log('🔄 Daily reset! Old:', gameState.dailyEarned, '→ New:', 0);
    ...
    console.log('📊 Daily Pool Info:', poolInfo);
    console.log('💰 Personal Daily Limit:', gameState.personalDailyLimit, 'TAMA');
...
function updateDailyEarnedUI() {
  ...
  if (dailyEarnedDisplay) {
    ...
    dailyEarnedDisplay.textContent = `${formatNumber(gameState.dailyEarned)} / ${formatNumber(
      gameState.personalDailyLimit
    )}`;
...
const EVOLUTION_STAGES = [
  { minLevel: 1, name: 'Egg', prefix: '🥚', sizeMultiplier: 0.7 },
  { minLevel: 5, name: 'Baby', prefix: '👶', sizeMultiplier: 0.85 },
  { minLevel: 15, name: 'Teen', prefix: '🌟', sizeMultiplier: 1.0 },
  { minLevel: 30, name: 'Adult', prefix: '⭐', sizeMultiplier: 1.15 },
  { minLevel: 50, name: 'Legend', prefix: '👑', sizeMultiplier: 1.3 },
];
...
console.log('%c🚀 GAME SCRIPT LOADING...', 'font-size: 16px; font-weight: bold; color: #00ff00;');
...
function updatePetEvolution() {
  ...
  if (currentStage && gameState.petType !== currentStage.emoji) {
    ...
    showMessage(`🎉 Your pet evolved into ${currentStage.name}!`);
  }
}

// ⚖️ DEGRADATION SYSTEM - Pets need care!
function degradeStats() {
  ...
  if (timeSinceLastDeg >= DEGRADATION_INTERVAL) {
    ...
    gameState.food = Math.max(
      0,
      gameState.food - (gameState.maxFood * DEGRADATION_RATES.food) / 100
    );
    gameState.happy = Math.max(
      0,
      gameState.happy - (gameState.maxHappy * DEGRADATION_RATES.happy) / 100
    );

    // Critical warnings
    if (gameState.food < 20) {
      showMessage('⚠️ Your pet is very hungry!');
      // Low food accelerates HP loss
      gameState.hp = Math.max(0, gameState.hp - (gameState.maxHp * 0.5) / 100);
    }

    if (gameState.happy < 20) {
      showMessage('😢 Your pet is very sad!');
    }
    ...
    updateUI();
    // ⚠️ Save via triggerAutoSave (which prioritizes Supabase)
    triggerAutoSave();
  }
}
...
function createParticleBurst(x, y, combo) {
  ...
  const particleTypes = [
    ...
    { class: 'blob-star', emoji: '✨', size: 18 },
    { class: 'blob-purple', emoji: '💜', size: 22 },
    { class: 'blob-glow', emoji: '', size: 10 },
    { class: 'blob-star', emoji: '⭐', size: 16 },
  ];
...
function clickPet(e) {
  // 🛡️ PREVENT DOUBLE-CLICK: Block if already processing or too soon
  const now = Date.now();
  if (isProcessingClick) {
    console.warn('⚠️ Click already processing, ignoring duplicate');
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  if (now - lastClickProcessTime < MIN_CLICK_INTERVAL) {
    console.warn('⚠️ Click too soon, ignoring (spam protection)');
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  if (!window.TELEGRAM_USER_ID && !window.WALLET_ADDRESS) {
    console.warn('⚠️ Authentication required to play');
    ...
    if (window.WalletAuth && !window.Telegram?.WebApp?.initDataUnsafe?.user) {
      ...
      if (modal) {
        ...
        showMessage('🔐 Please connect your wallet to play!', 'error');
      }
    } else {
      showMessage('🔐 Please open the game in Telegram or connect your wallet!', 'error');
    }
    setTimeout(() => {
  ...
  // ⚡ RESET CLICK EFFICIENCY TO 1.0 AT START OF EACH CLICK
  // This prevents accumulation of penalties from previous clicks
  let localClickEfficiency = 1.0;

  // ⚡ FATIGUE SYSTEM - Track clicks per minute
  ...
  if (clicksInWindow > MAX_CLICKS_PER_WINDOW) {
    ...
    if (clicksInWindow === MAX_CLICKS_PER_WINDOW + 1) {
      showMessage('😴 Your pet is tired! Efficiency reduced to 50%. Rest for 60 seconds.');
    }
  }
  ...
  if (gameState.hp <= 0) {
    ...
    showMessage('💀 Your pet is unconscious! Heal it first!');
    return;
  }
  ...
  if (gameState.food <= 0) {
    ...
    showMessage('🍽️ Your pet is starving! Feed it first!');
    return;
  }
  ...
  if (gameState.happy <= 0) {
    ...
    showMessage('😢 Your pet is too depressed! Play with it first!');
    return;
  }
  ...
  // ⚡ ENERGY COST - Every click consumes resources!
  ...
  if (now - gameState.lastClickTime < COMBO_COOLDOWN) {
    ...
    const decayFactor =
      1 - (now - gameState.lastClickTime - COMBO_WINDOW) / (COMBO_DECAY_TIME - COMBO_WINDOW);
  ...
  if (earnedTama <= 0 || isNaN(earnedTama) || !isFinite(earnedTama)) {
    console.error('⚠️ Invalid earnedTama calculated:', earnedTama, {
      clickPower,
      BASE_CLICK_REWARD,
      cappedComboBonus,
      baseReward,
  ...
  // 🎨 NFT BOOST: Apply sum of all active NFT multipliers
  // Formula: earnedTama *= (1 + userNFTBoost)
  // Example: 20x total multiplier → userNFTBoost = 19.0 → earnedTama *= 20.0
  if (window.userNFTBoost && window.userNFTBoost > 0) {
    ...
    if (gameState.combo === 1 || gameState.combo === 0) {
      console.log(
        `🖼️ NFT Boost Applied: ${earnedTamaBeforeBoost.toFixed(2)} TAMA × ${multiplier.toFixed(
          2
        )} = ${earnedTama.toFixed(2)} TAMA`
      );
    }
  }
  ...
  // ⏰ HALVING LIMIT CHECK - Check daily reset first
  checkDailyReset();
  ...
  if (availableToday <= 0) {
    ...
    showMessage('⏰ Daily limit reached! Come back tomorrow!', 'error');
    console.log(
      '🚫 Daily limit reached! Earned today:',
      gameState.dailyEarned,
      '/ Limit:',
      gameState.personalDailyLimit
    );
  ...
  if (earnedTama > availableToday) {
    console.log(
      `⚠️ Capping reward: ${earnedTama.toFixed(2)} → ${availableToday.toFixed(
        2
      )} TAMA (daily limit)`
    );
  ...
  // Play click sound (if sound enabled)
  playSound('click');
  ...
  if (vectorPet) {
    ...
    // Create particle burst effect (Native)
    createParticleBurst(e.clientX, e.clientY, gameState.combo);

    // 🎉 ULTRA COMBO VISUALS (UIJuice)
    if (gameState.combo >= 10 && window.UIJuice && window.UIJuice.createEmojiBurst) {
      window.UIJuice.createEmojiBurst(e.clientX, e.clientY, ['🔥', '⚡', '💰']);
      if (window.Telegram?.WebApp?.HapticFeedback) {
  ...
  if (gameState.combo >= 5) {
    displayText = `+${earnedTama.toFixed(1)} 🔥x${gameState.combo}`;
  }
...
function checkLevelUp() {
  if (gameState.xp >= gameState.xpToLevel) {
    ...
    // ⚠️ REMOVED: gameState.clickPower = gameState.level;
    ...
    showMessage(`🎊 LEVEL UP! You are now Level ${gameState.level}! Earned ${levelBonus} TAMA!`);
...
// 💰 PROGRESSIVE PRICING - Prices scale with level
...
function updateActionPrices() {
  ...
  feedBtn.innerHTML = `🍔<br>Feed<br><small>-${feedCost} TAMA</small>`;
  playBtn.innerHTML = `🎮<br>Play<br><small>-${playCost} TAMA</small>`;
  healBtn.innerHTML = `💊<br>Heal<br><small>-${healCost} TAMA</small>`;
}
...
if (healBtn) {
  healBtn.addEventListener('click', () => {
    ...
    if (gameState.tama >= cost) {
      ...
      showMessage('💊 Feeling better! HP +50');
      checkLevelUp();
      updateUI();
...
// Дебаунсинг для предупреждений (не показываем одно и то же предупреждение слишком часто)
...
setInterval(() => {
  ...
  if (timePassed > 0) {
    ...
    // NEW: Progressive warning system (с дебаунсингом)
    if (gameState.hp <= 0) {
      showWarning('hp-0', '💀 Your pet has fainted! Emergency healing needed!');
    } else if (gameState.hp < 10) {
      showWarning('hp-10', '🚨 CRITICAL! Your pet is dying! Heal immediately!');
    } else if (gameState.hp < 20) {
      showWarning('hp-20', '⚠️ DANGER! Your pet needs healing!');
    } else if (gameState.food <= 0) {
      showWarning('food-0', '🍽️ Your pet is starving! Feed it now!');
    } else if (gameState.food < 10) {
      showWarning('food-10', '⚠️ Your pet is very hungry! Feed it!');
    } else if (gameState.food < 20) {
      showWarning('food-20', '🍽️ Your pet is hungry! Feed it!');
    } else if (gameState.happy <= 0) {
      showWarning('happy-0', '😢 Your pet is severely depressed! Play with it!');
    } else if (gameState.happy < 20) {
      showWarning('happy-20', '😢 Your pet is very sad. Play with it!');
    } else if (gameState.happy < 30) {
      showWarning('happy-30', '😢 Your pet is sad. Play with it!');
    }
  }
  ...
  const achievements = [
    {
      id: 'first_click',
      name: 'First Steps',
      desc: 'Click your pet for the first time',
    ...
    },
    {
      ...
      name: 'Clicker',
      desc: 'Click 10 times',
      check: () => gameState.totalClicks === 10,
    },
    {
      id: 'clicker_100',
      name: 'Dedicated',
    ...
    },
    {
      id: 'rich_1000',
      name: 'Getting Rich',
      desc: 'Earn 1,000 TAMA',
      check: () => gameState.tama >= 1000,
    },
    {
      id: 'rich_10000',
      name: 'Wealthy',
      desc: 'Earn 10,000 TAMA',
      check: () => gameState.tama >= 10000,
    },
  ];
...
function checkQuests() {
  ...
  if (!gameState.questClicksCompleted && gameState.questClicks >= 50) {
    ...
    showMessage('🎯 Quest Complete! Click Master +500 TAMA');
  ...
  if (!gameState.questLevelCompleted && gameState.level >= 5) {
    ...
    showMessage('🎯 Quest Complete! Level Up! +1,000 TAMA');
  ...
  document.getElementById('quest-clicks-progress').textContent = `${Math.min(
    gameState.questClicks,
    50
  )}/50`;
  document.getElementById('quest-level-progress').textContent = `${gameState.level}/5`;
}

// Shop system
console.log('🛒 Setting up shop system...');

// Ensure only one shop button exists (avoid accidental duplicates in dev/refresh scenarios)
try {
  const shopBtns = document.querySelectorAll('#shop-btn');
  shopBtns.forEach((el, idx) => {
    if (idx > 0) el.remove();
  });
} catch (e) {
  // ignore
...
if (shopBtn && shopModal) {
  ...
  console.log('✅ Shop button listener added');
} else {
...
function buyItem(itemType, price) {
  ...
  if (window.WALLET_ADDRESS && window.WalletSave) {
    // Save for wallet users
    window.WalletSave.save(gameState).catch((err) =>
      console.warn('⚠️ Failed to save balance after shop purchase (wallet):', err)
    );
  } else if (window.TELEGRAM_USER_ID && typeof TAMA_API_BASE !== 'undefined') {
    // Save for Telegram users
    ...
    }).catch((err) => console.warn('⚠️ Failed to save balance after shop purchase:', err));
  }
}
...
function loadSkinsShop() {
  ...
  skins.forEach((skin) => {
    ...
    skinCard.innerHTML = `
                    ...
                    ${
                      skin.price > 0
                        ? `<div class="skin-price-shop">${skin.price.toLocaleString()} TAMA</div>`
                        : '<div class="skin-price-shop">FREE</div>'
                    }
...
function buySkin(skinId) {
  ...
  if (window.WALLET_ADDRESS && window.WalletSave) {
    // Save for wallet users
    window.WalletSave.save(gameState).catch((err) =>
      console.warn('⚠️ Failed to save balance after skin purchase (wallet):', err)
    );
  } else if (window.TELEGRAM_USER_ID && typeof TAMA_API_BASE !== 'undefined') {
    // Save for Telegram users
    fetch(`${TAMA_API_BASE}/leaderboard/upsert`, {
    ...
    }).catch((err) => console.warn('⚠️ Failed to save balance after skin purchase:', err));
  }
...
function loadOwnedSkinsShop() {
  ...
  ownedSkins.forEach((skinId) => {
    ...
    skinCard.innerHTML = `
                    <div class="rarity-badge rarity-${skin.rarity}">${
      skin.rarityLabel || skin.rarity.toUpperCase()
    }</div>
                    <div class="skin-preview" style="font-size: 80px; margin: 20px 0;">${
                      emojiMap[skin.id] || '🎨'
                    }</div>
                    <div class="skin-name">${skin.name}</div>
                    <div class="skin-price" style="color: #22c55e;">✓ Owned</div>
                    <div class="skin-bonus">${skin.bonusEn || skin.bonusRu}</div>
                    ${
                      isEquipped
                        ? '<button class="skin-buy-btn equipped">✓ Equipped</button>'
                        : '<button class="skin-buy-btn owned" onclick="equipSkin(\'' +
                          skin.id +
                          '\')">Equip</button>'
                    }
...
function openPetSelector() {
  ...
  Object.keys(PET_TYPES).forEach((petKey) => {
    ...
    petCard.style.cssText = `
                    background: ${
                      isSelected ? 'rgba(16, 185, 129, 0.3)' : 'rgba(74, 85, 104, 0.3)'
                    };
    ...
    petCard.innerHTML = `
                    ...
                    <div style="font-size: 12px; color: #fbbf24;">${
                      isOwned ? (isSelected ? '✓ Selected' : 'Select') : `${pet.price} TAMA`
                    }</div>
                `;
...
function selectPet(petKey, isOwned) {
  ...
  if (!isOwned) {
    ...
    showMessage(`🎉 You bought ${pet.name}!`);
  }
...
setInterval(() => {
  if (gameState.hasAutoFeeder && gameState.food < 30) {
    ...
    if (now - lastAutoFeed > 60000) {
      ...
      showMessage('💎 Auto-Feeder activated! +20 Food');
      lastAutoFeed = now;
      updateUI();
    }
  }
...
// 🎁 REFERRAL SYSTEM: Check for referral code on page load
  ...
  try {
    ...
    if (refCode && refCode.startsWith('TAMA')) {
      ...
      const referrerResponse = await fetch(
        `${SUPABASE_URL}/rest/v1/leaderboard?referral_code=eq.${refCode}&select=telegram_id,telegram_username`,
        {
          headers: {
      ...
      if (referrerData && referrerData.length > 0) {
        ...
        if (referrerId !== currentUserId) {
          ...
          const existingRefResponse = await fetch(
            `${SUPABASE_URL}/rest/v1/referrals?referrer_telegram_id=eq.${referrerId}&referred_telegram_id=eq.${currentUserId}`,
            {
              headers: {
                apikey: SUPABASE_ANON_KEY,
                Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
          ...
          if (existingRefs && existingRefs.length > 0) {
            console.log('✅ Referral already exists, skipping creation in game');
            console.log('📊 Referral was created by bot, game will only track progress');
          } else {
            console.log('⚠️ No referral found - this should have been created by bot');
            console.log('🔄 Game will not create referral records (bot handles this)');
          }

          // Always award TAMA to referrer (even if referral was created by bot)
          try {
            console.log('💰 Awarding TAMA to referrer...');

            // Get current TAMA balance
            const currentBalanceResponse = await fetch(
              `${SUPABASE_URL}/rest/v1/leaderboard?telegram_id=eq.${referrerId}&select=tama`,
              {
                headers: {
                  apikey: SUPABASE_ANON_KEY,
                  Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
            ...
            const currentTama =
              currentBalanceData && currentBalanceData.length > 0
                ? currentBalanceData[0].tama || 0
                : 0;
            const newTama = currentTama + 1000;

            // 🛡️ SECURITY: Use API instead of direct Supabase access to prevent cheating
            ...
            if (updateResponse.ok) {
              console.log('✅ TAMA awarded to referrer:', referrerId, '+1000 TAMA');
              showNotification('🎉 Referral successful! Your referrer earned 1,000 TAMA!');
            } else {
              console.log('❌ Failed to award TAMA to referrer');
            }
          } catch (error) {
            console.log('❌ Error awarding TAMA to referrer:', error);
          }
        } else {
...
async function initGame() {
  ...
  try {
    // ✅ УМНОЕ ОПРЕДЕЛЕНИЕ: Проверяем параметр URL для принудительного выбора метода
    const urlParams = new URLSearchParams(window.location.search);
    const forceAuth = urlParams.get('auth'); // 'telegram', 'wallet', или null

    // ✅ ПРИОРИТЕТ 1: Wallet авторизация (если принудительно или Telegram недоступен/не авторизован)
    // Проверяем не только наличие Telegram WebApp, но и наличие user ID
    const hasTelegramUser =
      window.Telegram?.WebApp?.initDataUnsafe?.user?.id ||
      (window.Telegram?.WebApp?.initData && window.Telegram.WebApp.initData.includes('user='));

    if (
      (forceAuth === 'wallet' || (forceAuth !== 'telegram' && !hasTelegramUser)) &&
      window.WalletAuth
    ) {
      ...
      if (walletInit.success && walletInit.walletAddress) {
        console.log('✅ Wallet authenticated:', walletInit.walletAddress);
        window.WALLET_ADDRESS = walletInit.walletAddress;
        window.WALLET_USER_ID =
          walletInit.userData?.user_id || 'wallet_' + walletInit.walletAddress.substring(0, 12);

        // ⚡ PARALLEL LOADING: Load referral and game state simultaneously
        const [referralResult, walletData] = await Promise.allSettled([
          // Referral processing (non-blocking)
          window.ReferralSystem
            ? window.ReferralSystem.processReferralBonus(walletInit.walletAddress)
            : Promise.resolve(),
          // Game state loading (critical)
          window.WalletSave && typeof window.WalletSave.load === 'function'
            ? window.WalletSave.load()
        ...
        if (walletData.status === 'fulfilled' && walletData.value && walletData.value.exists) {
          ...
          if (data.questClicksCompleted !== undefined)
            gameState.questClicksCompleted = data.questClicksCompleted;
          if (data.questLevelCompleted !== undefined)
            gameState.questLevelCompleted = data.questLevelCompleted;
          cloudLoaded = true;
          console.log('✅ Game state loaded from wallet');
        } else {
          console.log('ℹ️ No saved game state for wallet, starting fresh');
        }
        ...
        console.log('📱 Wallet connection needed');
        const modal = document.getElementById('wallet-connect-modal');
        if (modal) {
          modal.style.display = 'flex';
        }
    ...
    // ✅ ПРИОРИТЕТ 2: Telegram авторизация (если доступна и не принудительно wallet)
    if (!cloudLoaded && (forceAuth === 'telegram' || (!forceAuth && hasTelegramUser))) {
      ...
      if (window.Telegram && window.Telegram.WebApp) {
        ...
        if (user && user.id) {
          // ⚠️ CRITICAL: Always convert to string for consistency
          userId = String(user.id);
          console.log('✅ Method 1: Got user from initDataUnsafe:', {
            id: userId,
          ...
          if (initData) {
            console.log('🔍 Method 2: Trying to parse initData manually');
            try {
              ...
              if (userParam) {
                ...
                console.log('✅ Method 2: Parsed user from initData:', {
                  id: userId,
                  idType: typeof userId,
        ...
        if (userId) {
          // ⚠️ CRITICAL: Always store as string for consistency
          const userIdStr = String(userId);
          console.log('📥 Loading from Supabase for user:', userIdStr);
          console.log('🔍 User ID type check:', {
            original: userId,
            asString: userIdStr,
          ...
          // ⚡ Load rank immediately (no delay needed)
          ...
          console.warn('⚠️ Could not get user ID from Telegram WebApp');
          console.warn('   Offering wallet connection as alternative');
          // Telegram WebApp есть, но user ID не получен - предлагаем wallet
          if (!window.WALLET_ADDRESS && window.WalletAuth) {
            const modal = document.getElementById('wallet-connect-modal');
            if (modal && modal.style.display !== 'flex') {
              modal.style.display = 'flex';
              console.log('📱 Showing wallet connection modal (Telegram user ID not available)');
            }
          }
        }
      }
    } else {
      // Telegram WebApp недоступен - предлагаем wallet
      // Dev-mode (localhost + ?dev=1): allow forcing Telegram user id to load cloud saves
      try {
        const isLocalhost =
          window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
        const devEnabled =
          isLocalhost && new URLSearchParams(window.location.search).get('dev') === '1';
        const forcedUserId =
          devEnabled && window.TELEGRAM_USER_ID ? String(window.TELEGRAM_USER_ID) : '';
        if (!cloudLoaded && forcedUserId) {
          console.log('🧪 Dev auth: forcing Supabase load for user:', forcedUserId);
          cloudLoaded = await loadFromSupabase(forcedUserId);
        }
      } catch (e) {
        console.warn('Dev auth load skipped:', e);
      }

      if (!window.WALLET_ADDRESS && window.WalletAuth) {
        console.warn('⚠️ Telegram WebApp not available - offering wallet connection');
        const modal = document.getElementById('wallet-connect-modal');
        if (modal && modal.style.display !== 'flex') {
          modal.style.display = 'flex';
          console.log('📱 Showing wallet connection modal (no Telegram)');
        }
      } else if (window.WALLET_ADDRESS) {
        console.log('✅ Using wallet authentication (no Telegram needed)');
      } else {
        console.warn(
          '⚠️ No authentication method available - game will work with localStorage only'
        );
      }
    }
    ...
    if (!cloudLoaded) {
      console.log('💾 Loading from localStorage (fallback - Supabase failed or no user ID)');
      ...
      if (localLoaded) {
        ...
        console.log('⚠️ No data in localStorage, starting fresh game');
      ...
      console.log('✅ Using Supabase data (PRIMARY SOURCE) - localStorage ignored');
    ...
    // ⚡ PARALLEL: Update UI elements and load leaderboard simultaneously
    const uiUpdates = [
    ...
    if (cloudLoaded) {
      showMessage('☁️ Cloud progress loaded!');
    } else {
      showMessage('👋 Welcome! Click your pet to earn TAMA!');
    }

    // Check for first time player
    setTimeout(() => {
      if (gameState.totalClicks === 0) {
        showMessage('💡 TIP: Click rapidly for COMBO bonuses! 🔥');
      }
    }, 5000);
  } catch (error) {
    console.error('Init game error:', error);
    // ⚠️ IMPORTANT: Only load from localStorage if Supabase failed
    // If cloudLoaded was successful, don't overwrite with localStorage!
    if (!cloudLoaded) {
      console.log('💾 Supabase failed, loading from localStorage as fallback');
      loadFromLocalStorage();
    } else {
      console.log('✅ Supabase data already loaded, ignoring localStorage');
    }
    updateUI();
  }
}
...
async function loadFromSupabase(userId) {
  try {
    // ⚠️ CRITICAL: Ensure userId is always a string for consistent querying
    const userIdStr = String(userId);
    console.log(
      '🔍 Loading from Supabase with userId:',
      userIdStr,
      '(type:',
      typeof userIdStr,
      ')'
    ...
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/leaderboard?telegram_id=eq.${userIdStr}&select=*`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    console.log('📊 Found', data?.length || 0, 'record(s) for user', userIdStr);

    if (data && data.length > 0) {
      // ⚠️ If multiple records, use the most recent one (by last_activity or id)
      const userData =
        data.length > 1
      ...
      // ⚠️ CRITICAL: Verify telegram_id matches (in case of type mismatch)
      ...
      if (userData.telegram_username) {
        ...
        console.log('✅ Loaded custom username from Supabase:', gameState.customUsername);
      }
      ...
      console.log('📊 Current state: TAMA=' + gameState.tama + ', Level=' + gameState.level);

      // ⚠️ IMPORTANT: Save to localStorage as backup, but Supabase is source of truth
...
window.addEventListener('beforeunload', () => {
  // ⚠️ IMPORTANT: Wallet/Telegram is PRIMARY, localStorage is backup
  if (window.WALLET_ADDRESS && window.WalletSave) {
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    // ⚠️ IMPORTANT: Supabase is PRIMARY, localStorage is backup
    const userId = window.TELEGRAM_USER_ID;
    if (userId) {
      // Try to save to Supabase first (primary)
      saveDirectToSupabase(userId)
        .then(() => {
          // Save to localStorage as backup
          saveToLocalStorage();
        })
...
async function logTransaction(type, amount, balanceBefore, balanceAfter, metadata = {}) {
  ...
  const supabaseUrl =
    window.CONFIG?.SUPABASE_URL || typeof SUPABASE_URL !== 'undefined'
      ? SUPABASE_URL
      : 'https://zfrazyupameidxpjihrh.supabase.co';
  const supabaseKey =
    window.CONFIG?.SUPABASE_ANON_KEY || typeof SUPABASE_ANON_KEY !== 'undefined'
      ? SUPABASE_ANON_KEY
      : null;

  if (!supabaseKey) {
    console.warn('⚠️ Supabase API key not available, skipping transaction log');
    return;
  }

  try {
    // Get username: Telegram username or wallet address prefix
    const username =
      window.Telegram?.WebApp?.initDataUnsafe?.user?.username ||
      (window.WALLET_ADDRESS ? `Wallet_${window.WALLET_ADDRESS.substring(0, 8)}` : userId);
    ...
    const response = await fetch(`${supabaseUrl}/rest/v1/transactions`, {
      ...
      body: JSON.stringify({
        ...
        metadata: {
          ...metadata,
          wallet_address: window.WALLET_ADDRESS || null,
          account_type: window.WALLET_ADDRESS ? 'wallet' : 'telegram',
        },
      }),
    });

    if (response.ok) {
      console.log(`✅ Transaction logged: ${type} ${amount > 0 ? '+' : ''}${amount} TAMA`);
    } else {
      console.warn(`⚠️ Failed to log transaction: ${type}`, await response.text());
    }
  } catch (error) {
    console.warn('⚠️ Error logging transaction:', error);
  }
}
...
async function saveDirectToSupabase(userId) {
  try {
    // ⚠️ CRITICAL: Ensure userId is always a string for consistency
    const userIdStr = String(userId);

    // --- NEW: LOG PET CLICK TRANSACTIONS FOR DASHBOARD ---
    if (typeof window.lastLoggedClickCount === 'undefined') {
      window.lastLoggedClickCount = gameState.totalClicks;
    } else if (gameState.totalClicks - window.lastLoggedClickCount >= 10) {
      // Log if >10 clicks since last save
      const clickDiff = gameState.totalClicks - window.lastLoggedClickCount;
      window.lastLoggedClickCount = gameState.totalClicks;

      // Log transaction asynchronously (don't await) - uses global SUPABASE vars
      fetch(`${SUPABASE_URL}/rest/v1/transactions`, {
        ...
        body: JSON.stringify({
          ...
          wallet_address: window.walletAddress || null,
          type: 'click_session',
          amount: 0,
          metadata: { clicks: clickDiff, source: 'auto_save' },
        }),
      })
        .then((res) => {
          if (res.ok) console.log('✅ Click transaction logged:', clickDiff);
        })
        .catch((e) => console.warn('⚠️ Click log failed', e));
    }
    // -----------------------------------------------------

    console.log('💾 Saving to Supabase with userId:', userIdStr, '(type:', typeof userIdStr, ')');
    ...
    const defaultUsername =
      telegramUser?.username || telegramUser?.first_name || `User${userIdStr}`;

    // ⚠️ IMPORTANT: Use custom username if set, otherwise use Telegram username
    // This prevents overwriting custom names on auto-save
    const username = gameState.customUsername || defaultUsername;
    console.log('💾 Using username:', username, '(custom:', !!gameState.customUsername, ')');
    ...
    const requestBody = {
      user_id: userIdStr, // ⚠️ Always use string
      user_type: 'telegram',
      ...
      skip_transaction_log: false, // ⚠️ CRITICAL: Set to false to enable balance protection!
    };
...
function triggerAutoSave(forceCritical = false) {
  ...
  // ✅ ПРИОРИТЕТ 1: Wallet (для стран без Telegram)
  if (window.WALLET_ADDRESS && window.WalletSave) {
    console.log('💾 Saving via wallet:', {
      address: window.WALLET_ADDRESS,
      userId: window.WALLET_USER_ID,
    });

    // 📊 Track wallet activity for DappRadar
    ...
    saveDirectToSupabase(window.TELEGRAM_USER_ID)
      .then((success) => {
        if (success) {
          ...
          console.warn('⚠️ Auto-save to Supabase failed, saving to localStorage as backup');
          saveToLocalStorage();
        }
      })
      .catch((error) => {
...
function saveToLocalStorage() {
  try {
    ...
    // 🔑 Use user-specific key to prevent data conflicts between accounts
    const userId = window.TELEGRAM_USER_ID || window.WALLET_ADDRESS || 'default';
    const storageKey = `tamagotchi_save_${userId}`;
    localStorage.setItem(storageKey, JSON.stringify(saveData));
    console.log(`💾 Game saved to localStorage with key: ${storageKey}`);
  } catch (error) {
...
function loadFromLocalStorage() {
  try {
    // 🔑 Use user-specific key to load correct user's data
    const userId = window.TELEGRAM_USER_ID || window.WALLET_ADDRESS || 'default';
    const storageKey = `tamagotchi_save_${userId}`;
    ...
    if (saveData) {
      ...
      if (data.lastSave && Date.now() - data.lastSave < 7 * 24 * 60 * 60 * 1000) {
        ...
        // ⚠️ FIX: clickPower should always be 1.0 (economy controlled via Admin Panel)
        ...
        if (data.clickPower && data.clickPower > 2.0) {
          console.warn(`⚠️ Fixed clickPower: was ${data.clickPower}, reset to 1.0`);
        }
        gameState.petType = data.petType || '🐾';
        ...
        if (data.customUsername) {
          ...
          console.log('✅ Loaded custom username from localStorage:', data.customUsername);
        }
...
console.log('Mini games elements:', {
  minigamesBtn,
  minigamesModal,
  closeMinigames,
  gameSelection,
...
document.querySelectorAll('.game-card').forEach((card) => {
  card.addEventListener('click', (e) => {
    ...
    // Mapping games to external files
    const externalGames = {
      luckyslots: 'slots.html',
      wheel: 'wheel.html',
      platformer: 'super-tama-bros.html',
      colormatch: 'tama-color-match.html',
      shooter: 'tama-shooter.html',
    };

    if (externalGames[gameName]) {
      const userId = window.TELEGRAM_USER_ID || window.WALLET_USER_ID || '';
      const url = `/${externalGames[gameName]}?user_id=${userId}`;
      console.log(`🚀 Opening external game: ${url}`);

      // Try window.open first, fallback to location.href if it fails or if in TG
      try {
        const newWindow = window.open(url, '_blank');
        if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
          console.warn('Popup blocked or failed, using location.href');
    ...
    // Internal arena handling
    const arena =
      document.getElementById(`${gameName}-arena`) ||
      document.getElementById(`${gameName.replace('lucky', '')}-arena`);
    if (arena) {
      console.log(`🏟️ Opening arena: ${arena.id}`);
      gameSelection.style.display = 'none';
      arena.style.display = 'block';
      arena.classList.add('active');
    } else {
      console.error(`❌ Arena not found for game: ${gameName}`);
      showMessage('Game coming soon! 🚧');
    }
  });
});
...
function loadCareState() {
  const key = `tama_care_v1:${getCareUserKey()}`;
  try {
...
function saveCareState(key, state) {
  try {
...
function computeCareMood() {
  // Use current game stats, plus a small daily "temper".
  // Mood is not a stat bar; it's a state.
  const hpPct = gameState.maxHp ? gameState.hp / gameState.maxHp : 1;
  const foodPct = gameState.maxFood ? gameState.food / gameState.maxFood : 1;
  const happyPct = gameState.maxHappy ? gameState.happy / gameState.maxHappy : 1;

  if (hpPct < 0.2 || foodPct < 0.15) return '😢';
  if (happyPct < 0.25) return '😔';
  if (gameState.combo > 10) return '😡';
  if (happyPct > 0.85 && hpPct > 0.75 && foodPct > 0.7) return '🥰';
  if (gameState.totalClicks > 120) return '😲';
  return '😊';
}

function setCareScene(text, moodEmoji, ttlMs = 2200) {
  const el = document.getElementById('care-scene');
  if (!el) return;
  el.innerHTML = text;
  if (moodEmoji) {
    const careMood = document.getElementById('care-mood-emoji');
    if (careMood) careMood.textContent = moodEmoji;
    updateVectorPetEmotion(moodEmoji);
  }
  if (ttlMs > 0) {
...
function updateCareFaces(activeEmoji) {
  const map = {
    '😢': 1,
    '😔': 1,
    '😐': 2,
  ...
  const idx = map[activeEmoji] || 3;
  for (let i = 1; i <= 5; i++) {
    const el = document.getElementById(`care-face-${i}`);
    if (el) el.classList.toggle('active', i === idx);
  }
}

function initCareRitual() {
  ...
  const closeBtn = document.getElementById('care-close-btn');
  const feed = document.getElementById('care-feed-btn');
  const pet = document.getElementById('care-pet-btn');
  const heal = document.getElementById('care-heal-btn');
  const sleep = document.getElementById('care-sleep-btn');

  if (!careBtn || !careModal || !carePanel) return;

  const { key, state: loaded } = loadCareState();
  ...
  if (state.dayKey !== today) {
    // Streak logic: if last action was yesterday, keep streak; otherwise reset.
    if (state.lastDayWithAction === yesterday) {
      state.streak = Math.min(999, (state.streak || 0) + 1);
    } else {
      state.streak = 0;
    }
    state.dayKey = today;
  ...
  // Soft "miss you" if absent
  const now = Date.now();
  if (state.lastVisitTs && now - state.lastVisitTs > 1000 * 60 * 60 * 24) {
    setCareScene('He looks at you like: <strong>"Where were you?"</strong>', '😔', 2600);
  }
  state.lastVisitTs = now;

  function persist() {
    saveCareState(key, state);
  }

  function refreshCareUI() {
    const usedEl = document.getElementById('care-actions-used');
    const maxEl = document.getElementById('care-actions-max');
    const moodEl = document.getElementById('care-mood-emoji');

    if (usedEl) usedEl.textContent = String(state.usedToday || 0);
    if (maxEl) maxEl.textContent = String(state.dailyMax || 3);

    const mood = computeCareMood();
    if (moodEl) moodEl.textContent = mood;
    updateCareFaces(mood);

    const exhausted = (state.usedToday || 0) >= (state.dailyMax || 3);
    if (feed) feed.disabled = exhausted;
    if (pet) pet.disabled = exhausted;
    if (heal) heal.disabled = exhausted;
    if (sleep) sleep.disabled = exhausted;
  }

  function consumeAction() {
    state.usedToday = Math.min(state.dailyMax || 3, (state.usedToday || 0) + 1);
    state.lastDayWithAction = today;
    state.bond = Math.min(1000000, (state.bond || 0) + 1);

    // Unlocks (purely cosmetic/ritual)
    if (state.streak >= 2 && !state.unlocks.halo) {
      state.unlocks.halo = true;
      setCareScene('Unlocked: <strong>Halo</strong> ✨ (you’ve been gentle for days)', '🥰', 3200);
    }
    if (state.streak >= 6 && !state.unlocks.sparkle) {
      state.unlocks.sparkle = true;
      setCareScene('Unlocked: <strong>Sparkle</strong> 💫 (habit becomes memory)', '🥰', 3200);
    }
  }

  function canAct() {
    const now = Date.now();
    if (now - (state.lastActionTs || 0) < 12000) {
      setCareScene('He needs a breath. <strong>Wait a little.</strong>', '😐', 1800);
      return false;
    }
    if ((state.usedToday || 0) >= (state.dailyMax || 3)) {
      setCareScene('That’s enough for today. <strong>Come back tomorrow.</strong>', '🥰', 2400);
      return false;
    }
  ...
  function doCare(action) {
    ...
    if (action === 'pet') {
      // tiny, safe stat nudge (no economy impact)
      gameState.happy = Math.min(gameState.maxHappy, gameState.happy + 3);
      updateUI();
      setCareScene('You pet him. He leans in. <strong>...</strong>', '😘', 2200);
      triggerAutoSave();
      return;
    }
    ...
    if (action === 'sleep') {
      ...
      const atNight = hour >= 22 || hour <= 6;
      const bonus = atNight ? 8 : 4;
      gameState.happy = Math.min(gameState.maxHappy, gameState.happy + bonus);
      updateUI();
      setCareScene(
        atNight
          ? 'You tuck him in. <strong>Soft breathing…</strong>'
          : 'A short nap. <strong>He calms down.</strong>',
        '😵',
        2400
      );
      triggerAutoSave();
    ...
    if (action === 'feed') {
      const variants = [
        { food: '🍓', line: 'A sweet bite. He smiles.', mood: '😊' },
        { food: '🥦', line: 'He makes a face… then eats anyway.', mood: '😐' },
        { food: '🍪', line: 'Too tasty. He gets excited!', mood: '😋' },
      ];
      const v = variants[Math.floor(Math.random() * variants.length)];
      setCareScene(`You offer ${v.food}. <strong>${v.line}</strong>`, v.mood, 2400);

      // Reuse existing economy-safe action
    ...
    if (action === 'heal') {
      // A tiny scene: sad -> okay -> love
      setCareScene('He looks weak… <strong>you stay.</strong>', '😢', 1400);
      setTimeout(() => setCareScene('Breathing gets steady.', '😊', 1400), 900);
      setTimeout(() => setCareScene('He’s back. <strong>Thank you.</strong>', '🥰', 2200), 1700);
    ...
    // fallback
    setCareScene(`Care done.`, mood, 1600);
  }

  function openCare() {
    careModal.classList.add('show');
  ...
  function closeCare() {
    careModal.classList.remove('show');
    careModal.setAttribute('aria-hidden', 'true');
    careBtn.classList.remove('active');
  }

  careBtn.addEventListener('click', () => {
    const isOpen = careModal.classList.contains('show');
    if (isOpen) closeCare();
    else openCare();
  });

  if (closeBtn) closeBtn.addEventListener('click', closeCare);

  // Close on background click
  careModal.addEventListener('click', (e) => {
    if (e.target && e.target.id === 'care-modal') {
      closeCare();
    }
  });
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && careModal.classList.contains('show')) {
      closeCare();
    }
  });

  if (feed) feed.addEventListener('click', () => doCare('feed'));
  if (pet) pet.addEventListener('click', () => doCare('pet'));
  if (heal) heal.addEventListener('click', () => doCare('heal'));
  if (sleep) sleep.addEventListener('click', () => doCare('sleep'));

  refreshCareUI();
  persist();
}

// Init after core DOM is ready (file is loaded late, but safe to call)
try {
  initCareRitual();
} catch (e) {
  console.warn('Care ritual init failed', e);
}
...
function startMemoryGame() {
  ...
  cards.forEach((emoji, index) => {
    ...
    card.style.cssText =
      'background: #4a5568; border-radius: 8px; padding: 20px; font-size: 32px; text-align: center; cursor: pointer; transition: all 0.3s;';
...
function checkMatch() {
  ...
  if (first.emoji === second.emoji) {
    ...
    if (matchedPairs === 8) {
      ...
      updateUI();
      showMessage(`🎉 YOU WIN! +${reward} TAMA!`);
      triggerAutoSave();
    }
  } else {
...
function rollDice() {
  ...
  const interval = setInterval(() => {
    ...
    if (rolls === 10) {
      ...
      diceDisplay.textContent = ['⚀', '⚁', '⚂', '⚃', '⚄', '⚅'][finalDice - 1];

      const reward = finalDice * 50;
      gameState.tama += reward;
      updateUI();
      diceResult.textContent = `You rolled ${finalDice}! Won ${reward} TAMA! 🎉`;
      triggerAutoSave();
    }
  }, 100);
}
...
function spinSlots() {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'spend_slots',
      -100,
      'Spent 100 TAMA to play Slot Machine',
      gameState
    );
  }
  ...
  let spins = 0;
  const interval = setInterval(() => {
    slotsDisplay.innerHTML = symbols
      .map(() => `<div>${symbols[Math.floor(Math.random() * symbols.length)]}</div>`)
      .join('');
    spins++;
    ...
    if (spins === 15) {
      ...
      if (result[0] === result[1] && result[1] === result[2]) {
        reward = result[0] === '7️⃣' ? 1000 : result[0] === '💎' ? 500 : 200;
        slotsResult.textContent = `🎉 JACKPOT! +${reward} TAMA!`;
      } else if (result[0] === result[1] || result[1] === result[2]) {
        reward = 50;
        slotsResult.textContent = `Nice! +${reward} TAMA!`;
      } else {
        slotsResult.textContent = `Try again! 😔`;
      }
      ...
      if (typeof logTransaction === 'function' && reward > 0) {
        logTransaction(
          ...
          'earn_slots',
          reward,
          `Won ${reward} TAMA from Slot Machine`,
          gameState
        );
      }
...
// 🎰 SLOT MACHINE 2.0 - IMPROVED WITH RTP CONTROL
let slotsState = {
  betAmount: 100,
  spinning: false,
  wins: 0,
...
const slotSymbols = [
  ...
  { symbol: '🔔', weight: 10, payout: 5 },
  { symbol: '💰', weight: 8, payout: 6 },
  { symbol: '💎', weight: 5, payout: 8 },
  { symbol: '7️⃣', weight: 2, payout: 10 },
];
...
function updateSlotsBalance() {
  document.getElementById(
    'slots-balance-display'
  ).textContent = `${gameState.tama.toLocaleString()} TAMA`;
...
document.getElementById('slots-fullscreen-btn')?.addEventListener('click', () => {
  ...
  if (!slotsState.isFullscreen) {
    ...
    document.getElementById('slots-fullscreen-btn').textContent = '✕';
  } else {
...
document.getElementById('slots-spin-btn')?.addEventListener('click', () => {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'spend_slots',
      -slotsState.betAmount,
      `Spent ${slotsState.betAmount} TAMA to play Slot Machine 2.0`,
      gameState
    );
  }
  updateUI();
      ...
      if (reward > 0) {
        ...
        if (typeof logTransaction === 'function') {
          logTransaction(
            ...
            'earn_slots',
            reward,
            `Won ${reward} TAMA from Slot Machine 2.0`,
            gameState
          );
        }
        // Win animation
...
document.getElementById('wheel-fullscreen-btn')?.addEventListener('click', () => {
  ...
  if (!wheelState.isFullscreen) {
    ...
    document.getElementById('wheel-fullscreen-btn').textContent = '✕';
  } else {
    if (document.exitFullscreen) document.exitFullscreen();
    else if (document.webkitExitFullscreen) document.webkitExitFullscreen();
    else if (document.msExitFullscreen) document.msExitFullscreen();
...
document.getElementById('wheel-spin-btn')?.addEventListener('click', () => {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'spend_wheel',
      -wheelState.betAmount,
      `Spent ${wheelState.betAmount} TAMA to play Lucky Wheel`,
      gameState
    );
  }
  updateUI();
  ...
  function animateWheel() {
    ...
    if (progress < 1) {
      ...
      sounds.reelStop(); // Stop sound
      // ✅ FIX: Правильный расчет сегмента под указателем
      // Указатель наверху (0°), сегменты идут по часовой стрелке
      // Нормализуем rotation к [0, 2π]
      const normalizedRotation =
        ((wheelState.rotation % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
      // Угол одного сегмента
      const segmentAngle = (Math.PI * 2) / wheelState.segments.length;
      // Указатель наверху, нужно найти сегмент под ним
      // Инвертируем (т.к. колесо вращается против часовой, а сегменты по часовой)
      // Добавляем segmentAngle/2 чтобы указатель был в центре сегмента
      const segmentIndex =
        Math.floor((Math.PI * 2 - normalizedRotation + segmentAngle / 2) / segmentAngle) %
        wheelState.segments.length;
      ...
      if (multiplier >= 5) {
        wheelResult.textContent = `🎰 MEGA WIN! ${multiplier}x = +${reward} TAMA!`;
        wheelResult.style.color = '#fbbf24';
        sounds.jackpot(); // 🔊 JACKPOT!
      } else if (multiplier >= 3) {
        wheelResult.textContent = `⭐ BIG WIN! ${multiplier}x = +${reward} TAMA!`;
        wheelResult.style.color = '#10b981';
        sounds.bigWin(); // 🔊 BIG WIN!
      } else if (multiplier >= 2) {
        wheelResult.textContent = `✨ Nice! ${multiplier}x = +${reward} TAMA!`;
        wheelResult.style.color = '#3b82f6';
        sounds.win(); // 🔊 WIN!
      } else if (multiplier === 1) {
        wheelResult.textContent = `😐 Break Even! ${multiplier}x = ${reward} TAMA`;
        wheelResult.style.color = '#6b7280';
        sounds.reelStop(); // 🔊 Neutral sound
      } else {
        wheelResult.textContent = `😔 ${multiplier}x = ${reward} TAMA (lost ${
          wheelState.betAmount - reward
        })`;
      ...
      if (reward > wheelState.betAmount && typeof logTransaction === 'function') {
        logTransaction(
          ...
          'earn_wheel',
          reward,
          `Won ${reward} TAMA from Lucky Wheel (${multiplier}x)`,
          gameState
        );
      }
...
// 🎲 DICE ROLL - REMOVED (not interesting)

// 🐾 CATCH THE PET - IMPROVED VERSION
let catchPetState = {
  active: false,
  petX: 50,
  petY: 50,
...
document.getElementById('catchpet-start-btn')?.addEventListener('click', () => {
  ...
  catchPetState.petInterval = setInterval(() => {
    ...
    document.querySelector(
      '#catchpet-arena .game-arena > div > div:nth-child(3) > div:first-child'
    ).textContent = catchPetState.speed.toFixed(1) + 'x';
  }, 100);
...
function catchPetClick(e) {
  ...
  const distance = Math.sqrt(
    Math.pow(clickX - catchPetState.petX, 2) + Math.pow(clickY - catchPetState.petY, 2)
  );
  ...
  if (distance < 10) {
    ...
    // Update displays
    document.querySelector(
      '#catchpet-arena .game-arena > div > div:nth-child(1) > div:first-child'
    ).textContent = catchPetState.caught;
    document.querySelector(
      '#catchpet-arena .game-arena > div > div:nth-child(2) > div:first-child'
    ).textContent = catchPetState.score;
    ...
    // Visual feedback
    const status = document.getElementById('catchpet-status');
    status.textContent = `🎉 Caught! +${Math.floor(50 * catchPetState.speed)} points!`;
    status.style.color = '#10b981';

    setTimeout(() => {
      if (catchPetState.active) {
        status.textContent = `⏰ Time: ${catchPetState.gameTime}s | Caught: ${catchPetState.caught}`;
...
function endCatchPetGame() {
  ...
  if (totalReward > 0) {
    ...
    if (typeof logTransaction === 'function') {
      logTransaction(
        ...
        'earn_catchpet',
        totalReward,
        `Won ${totalReward} TAMA from Catch the Pet (${catchPetState.caught} caught)`,
        gameState
      );
    }
  }
  ...
  // Reset button
  document.getElementById('catchpet-start-btn').textContent = '🐾 START CATCHING!';
}
...
async function loadBattleNFTs() {
  ...
  try {
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/user_nfts?telegram_id=eq.${userId}&select=id,tier_name,rarity,earning_multiplier,nft_mint_address,is_active&limit=50`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (response.ok) {
      ...
      return nfts.filter((n) => {
        const isOnChain =
          n.nft_mint_address && n.nft_mint_address.length > 30 && !n.nft_mint_address.includes('_');
...
document.getElementById('battle-start-btn')?.addEventListener('click', async () => {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'spend_battle',
      -petBattleState.betAmount,
      `Spent ${petBattleState.betAmount} TAMA to battle`,
      gameState
    );
  }
  updateUI();
  ...
  playerPetDiv.querySelector('div:first-child').textContent = getPetEmoji(
    petBattleState.playerPet.tier_name
  );
  ...
  enemyPetDiv.querySelector('div:first-child').textContent = getPetEmoji(
    petBattleState.enemyPet.tier_name
  );
  enemyStats.textContent = `${petBattleState.enemyPet.tier_name} ${
    petBattleState.enemyPet.rarity
  } (${petBattleState.enemyPet.earning_multiplier.toFixed(1)}x)`;

  battleLog.innerHTML = '<div style="color: #3b82f6;">⚔️ Battle Started!</div>';

  // Simulate battle
  setTimeout(() => {
    const playerPower =
      petBattleState.playerPet.earning_multiplier +
      (petBattleState.playerPet.rarity === 'Legendary'
        ? 2
        : petBattleState.playerPet.rarity === 'Epic'
        ? 1
        : 0);
    const enemyPower =
      petBattleState.enemyPet.earning_multiplier +
      (petBattleState.enemyPet.rarity === 'Legendary'
        ? 2
        : petBattleState.enemyPet.rarity === 'Epic'
        ? 1
        : 0);
    ...
    battleLog.innerHTML += `<div style="color: #fff; margin-top: 5px;">Player attacks: ${playerRoll.toFixed(
      1
    )} power</div>`;
    battleLog.innerHTML += `<div style="color: #fff; margin-top: 5px;">Enemy attacks: ${enemyRoll.toFixed(
      1
    )} power</div>`;
    ...
    setTimeout(() => {
      ...
      if (won) {
        ...
        resultDiv.textContent = `🏆 VICTORY! +${reward} TAMA!`;
        resultDiv.style.color = '#10b981';
        battleLog.innerHTML += `<div style="color: #10b981; margin-top: 5px; font-weight: bold;">🏆 VICTORY! You won ${reward} TAMA!</div>`;
        ...
        if (typeof logTransaction === 'function') {
          logTransaction(
            ...
            'earn_battle',
            reward,
            `Won ${reward} TAMA from Pet Battle`,
            gameState
          );
        }
      } else {
        petBattleState.losses++;
        resultDiv.textContent = `💀 DEFEAT! Lost ${petBattleState.betAmount} TAMA`;
        resultDiv.style.color = '#ef4444';
        battleLog.innerHTML += `<div style="color: #ef4444; margin-top: 5px; font-weight: bold;">💀 DEFEAT! You lost ${petBattleState.betAmount} TAMA</div>`;
      }
...
function updateBattleBalance() {
  ...
  document.getElementById('battle-total-won').textContent =
    petBattleState.totalWon.toLocaleString();
}
...
document.getElementById('tower-start-btn')?.addEventListener('click', () => {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'spend_tower',
      -towerState.betAmount,
      `Spent ${towerState.betAmount} TAMA to build tower`,
      gameState
    );
  }
  updateUI();
  ...
  resultDiv.textContent = '🏗️ Start building! Click ADD BLOCK or CASH OUT!';
...
document.getElementById('tower-add-btn')?.addEventListener('click', () => {
  ...
  const resultDiv = document.getElementById('tower-result');
  resultDiv.textContent = `🏗️ Tower: ${towerState.blocks} blocks | Fall chance: ${(
    towerState.fallChance * 100
  ).toFixed(1)}%`;
  resultDiv.style.color = '#fff';
});
...
function renderTower() {
  ...
  for (let i = 0; i < towerState.blocks; i++) {
    ...
    block.style.cssText =
      'width: 60px; height: 30px; background: linear-gradient(135deg, #8b5cf6, #7c3aed); border: 2px solid #fff; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.3);';
...
function endTowerGame(fell) {
  ...
  if (fell) {
    const resultDiv = document.getElementById('tower-result');
    resultDiv.textContent = `💥 TOWER FELL! Lost ${towerState.betAmount} TAMA!`;
    resultDiv.style.color = '#ef4444';
  } else {
    ...
    const resultDiv = document.getElementById('tower-result');
    resultDiv.textContent = `💰 CASHED OUT! Tower: ${towerState.blocks} blocks | Reward: +${reward} TAMA (${multiplier}x)!`;
    resultDiv.style.color = '#10b981';
    ...
    if (typeof logTransaction === 'function') {
      logTransaction(
        ...
        'earn_tower',
        reward,
        `Won ${reward} TAMA from TAMA Tower (${towerState.blocks} blocks)`,
        gameState
      );
    }
  }
...
document.getElementById('precision-start-btn')?.addEventListener('click', () => {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'spend_precision',
      -precisionState.betAmount,
      `Spent ${precisionState.betAmount} TAMA to play Precision Click`,
      gameState
    );
  }
  updateUI();
  ...
  resultDiv.textContent = '🎯 Click when target is in the center!';
  resultDiv.style.color = '#fff';
...
function precisionClickHandler(e) {
  ...
  const distance = Math.sqrt(Math.pow(clickX - 50, 2) + Math.pow(clickY - 50, 2));
  ...
  if (reward > 0) {
    ...
    if (typeof logTransaction === 'function') {
      logTransaction(
        ...
        'earn_precision',
        reward,
        `Won ${reward} TAMA from Precision Click`,
        gameState
      );
    }
  }
...
function updatePrecisionBalance() {
  document.getElementById(
    'precision-balance-display'
  ).textContent = `${gameState.tama.toLocaleString()} TAMA`;
  document.getElementById('precision-perfect').textContent = precisionState.perfect;
  document.getElementById('precision-total-won').textContent =
    precisionState.totalWon.toLocaleString();
}
...
document.querySelectorAll('.roulette-bet-type').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    document
      .querySelectorAll('.roulette-bet-type')
      .forEach((b) => (b.style.border = '2px solid rgba(255,255,255,0.3)'));
...
document.getElementById('roulette-spin-btn')?.addEventListener('click', () => {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'spend_roulette',
      -rouletteState.betAmount,
      `Spent ${rouletteState.betAmount} TAMA on ${rouletteState.betType}`,
      gameState
    );
  }
  updateUI();
  ...
  setTimeout(() => {
    ...
    if (rouletteState.betType === 'red' && winningNumber.color === 'red') {
      ...
      rouletteState.betType === 'even' &&
      winningNumber.num > 0 &&
      winningNumber.num % 2 === 0
    ) {
      won = true;
      multiplier = 2;
    } else if (
      rouletteState.betType === 'odd' &&
      winningNumber.num > 0 &&
      winningNumber.num % 2 === 1
    ) {
      won = true;
      multiplier = 2;
    }
    ...
    if (won) {
      ...
      resultDiv.textContent = `🎉 WIN! Number: ${winningNumber.num} (${winningNumber.color}) | +${reward} TAMA!`;
      resultDiv.style.color = '#10b981';
      ...
      if (typeof logTransaction === 'function') {
        logTransaction(
          ...
          'earn_roulette',
          reward,
          `Won ${reward} TAMA from Roulette (${rouletteState.betType})`,
          gameState
        );
      }
    } else {
      rouletteState.losses++;
      resultDiv.textContent = `😔 LOSE! Number: ${winningNumber.num} (${winningNumber.color}) | Lost ${rouletteState.betAmount} TAMA`;
      resultDiv.style.color = '#ef4444';
    }
...
function updateRouletteBalance() {
  ...
  document.getElementById('roulette-total-won').textContent =
    rouletteState.totalWon.toLocaleString();
}

setTimeout(() => updateRouletteBalance(), 100);

// 🃏 CARD GAME (Durak/Blackjack style)
let cardsState = {
  betAmount: 100,
  active: false,
  playerCards: [],
...
document.getElementById('cards-start-btn')?.addEventListener('click', () => {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'spend_cards',
      -cardsState.betAmount,
      `Spent ${cardsState.betAmount} TAMA to play Card Game`,
      gameState
    );
  }
  updateUI();
...
document.getElementById('cards-hit-btn')?.addEventListener('click', () => {
  ...
  const newDeck = deck.filter(
    (card) => !allCards.some((c) => c.suit === card.suit && c.value === card.value)
  );
...
document.getElementById('cards-stand-btn')?.addEventListener('click', () => {
  ...
  endCardGame(
    cardsState.playerScore <= 21 &&
      (cardsState.dealerScore > 21 || cardsState.playerScore > cardsState.dealerScore)
  );
});
...
function renderCards() {
  ...
  cardsState.playerCards.forEach((card) => {
    ...
    cardDiv.style.cssText = `width: 60px; height: 90px; background: #fff; border-radius: 8px; padding: 5px; display: flex; flex-direction: column; justify-content: space-between; font-weight: bold; color: ${
      isRed ? '#ef4444' : '#000'
...
function endCardGame(won) {
  ...
  if (won) {
    ...
    resultDiv.textContent = `🏆 WIN! Your: ${cardsState.playerScore} | Dealer: ${cardsState.dealerScore} | +${reward} TAMA!`;
    resultDiv.style.color = '#10b981';
    ...
    if (typeof logTransaction === 'function') {
      logTransaction(
        ...
        'earn_cards',
        reward,
        `Won ${reward} TAMA from Card Game`,
        gameState
      );
    }
  } else {
    cardsState.losses++;
    resultDiv.textContent = `💀 LOSE! Your: ${cardsState.playerScore} | Dealer: ${cardsState.dealerScore} | Lost ${cardsState.betAmount} TAMA`;
    resultDiv.style.color = '#ef4444';
  }
...
// 🍄 TAMA JUMP PLATFORMER GAME - REAL MARIO STYLE
const platformerGame = {
  ...
  drawUI() {
    ...
    // Coins
    this.ctx.strokeText(`💰 ${this.coinsCollected}`, 20, 40);
    this.ctx.fillText(`💰 ${this.coinsCollected}`, 20, 40);

    // Lives
    this.ctx.strokeText(`❤️ ${this.lives}`, 160, 40);
    this.ctx.fillText(`❤️ ${this.lives}`, 160, 40);

    // Time
    this.ctx.strokeText(`⏱️ ${this.gameTime}s`, 300, 40);
    this.ctx.fillText(`⏱️ ${this.gameTime}s`, 300, 40);

    // Score
    this.ctx.strokeText(`★ ${this.score}`, 480, 40);
    this.ctx.fillText(`★ ${this.score}`, 480, 40);
  ...
  checkCollisions() {
    ...
    this.enemies.forEach((enemy) => {
      if (
        ...
        // Jump on enemy from TOP (kill it) - прыжок сверху
        if (this.player.vy > 0 && this.player.y + this.player.height - 15 < enemy.y + 8) {
          enemy.dead = true;
          this.player.vy = -8; // Отскок после убийства
          this.score += 20;
        ...
        // Касание сбоку/снизу - урон (но ПРОХОДИМ сквозь!)
        else {
          enemy.justHit = true; // Cooldown чтобы не умереть 100 раз подряд
          this.loseLife();
          setTimeout(() => {
            enemy.justHit = false;
          }, 1000);
    ...
    if (typeof TransactionLogger !== 'undefined') {
      ...
      if (userId && typeof TransactionLogger !== 'undefined') {
        TransactionLogger.logEarn(
          userId,
          window.Telegram?.WebApp?.initDataUnsafe?.user?.username ||
            window.WALLET_ADDRESS?.substring(0, 8) ||
            'user',
          'platformer_complete',
          totalReward,
          gameState.tama - totalReward,
  ...
  draw() {
    ...
    if (this.finishFlag) {
      ...
      this.ctx.fillRect(
        this.finishFlag.x,
        this.finishFlag.y,
        this.finishFlag.width,
        this.finishFlag.height
      );
      // Flag
  ...
  endGame() {
    ...
    if (this.coinsCollected >= 40) {
      reward = 500;
      message = '🏆 AMAZING! +500 TAMA!';
    } else if (this.coinsCollected >= 30) {
      reward = 300;
      message = '🎉 GREAT! +300 TAMA!';
    } else if (this.coinsCollected >= 20) {
      reward = 150;
      message = '👍 GOOD! +150 TAMA!';
    } else if (this.coinsCollected >= 10) {
      reward = 50;
      message = '😊 Not bad! +50 TAMA';
    } else {
      message = '😢 Try again! 0 TAMA';
    }
...
function closePlatformerFullscreen() {
  ...
  if (modal) {
    ...
    // Нет "normal canvas", остаемся на fullscreen
    platformerFullscreenMode = false;
  }
}
...
function startRace() {
  ...
  const speeds = [Math.random() * 3 + 2, Math.random() * 3 + 2, Math.random() * 3 + 2];
  ...
  const interval = setInterval(() => {
    ...
    if (Math.max(...positions) >= 300) {
      ...
      if (winner === 0) {
        const reward = 750;
        gameState.tama += reward;
        raceResult.textContent = `🎉 YOUR PET WON! +${reward} TAMA!`;
      } else {
        raceResult.textContent = `😔 You lost! Better luck next time!`;
      }
...
function startMatchmaking(betAmount) {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      ...
      'spend_battle_bet',
      -betAmount,
      `Placed ${betAmount} TAMA bet for PVP Battle`,
      gameState
    );
  }
...
function cancelMatchmaking() {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      window.TELEGRAM_USER_ID || 'unknown',
      'refund_battle_bet',
      battleState.betAmount,
      `Refunded ${battleState.betAmount} TAMA (cancelled matchmaking)`,
      gameState
    );
  }
...
function startBattleWithBot() {
  ...
  // Set player names
  document.getElementById('battle-player1-name').textContent =
    window.Telegram?.WebApp?.initDataUnsafe?.user?.first_name || 'You';
...
function resolveBattleRound() {
  ...
  const moveEmojis = { fast: '⚡', strong: '💪', block: '🛡️' };
  document.getElementById(
    'battle-round-result'
  ).textContent = `${moveEmojis[p1]} vs ${moveEmojis[p2]} - ${result}`;
  ...
  if (
    battleState.currentRound >= 5 ||
    battleState.player1Score >= 3 ||
    battleState.player2Score >= 3
  ) {
...
function endBattle() {
  ...
  if (won) {
    ...
    if (typeof logTransaction === 'function') {
      logTransaction(
        ...
        'earn_battle_win',
        prize,
        `Won ${prize} TAMA from PVP Battle`,
        gameState
      );
    }
  } else {
    ...
    document.getElementById(
      'battle-result-text'
    ).textContent = `You lost ${battleState.betAmount} TAMA...`;
    ...
    if (typeof logTransaction === 'function') {
      logTransaction(
        ...
        'lose_battle',
        0,
        `Lost ${battleState.betAmount} TAMA bet in PVP Battle`,
        gameState
      );
    }
  }
  ...
  document.getElementById(
    'battle-final-score'
  ).textContent = `${battleState.player1Score}-${battleState.player2Score}`;
...
function startRacingGame(betAmount) {
  ...
  if (typeof logTransaction === 'function') {
    logTransaction(
      ...
      'spend_racing_bet',
      -betAmount,
      `Placed ${betAmount} TAMA bet for Pet Racing`,
      gameState
    );
  }
...
function completeLap() {
  ...
  if (racingState.lap > racingState.maxLaps) {
    ...
    document.getElementById(
      'racing-status'
    ).textContent = `Lap ${racingState.lap} completed! Keep racing!`;
  }
}
...
function endRace() {
  ...
  if (won) {
    document.getElementById('racing-result-title').textContent = '🏆 VICTORY!';
    document.getElementById('racing-result-icon').textContent = '🏆';
    ...
    if (typeof logTransaction === 'function') {
      logTransaction(
        ...
        'earn_racing_win',
        reward,
        `Won ${reward} TAMA from Pet Racing (${getPositionText(finalPosition)} place)`,
        gameState
      );
    }
  } else {
    document.getElementById('racing-result-title').textContent = '💀 DEFEAT!';
    document.getElementById('racing-result-icon').textContent = '💀';
    document.getElementById(
      'racing-result-text'
    ).textContent = `You lost ${racingState.betAmount} TAMA...`;
    ...
    if (typeof logTransaction === 'function') {
      logTransaction(
        ...
        'lose_racing',
        0,
        `Lost ${racingState.betAmount} TAMA bet in Pet Racing (${getPositionText(
          finalPosition
        )} place)`,
        gameState
      );
  ...
  document.getElementById('racing-best-lap').textContent =
    racingState.lapTimes.length > 0
      ? (Math.min(...racingState.lapTimes) / 1000).toFixed(1) + 's'
      : 'N/A';
...
function throwDart() {
  ...
  gameState.tama += reward;
  updateUI();
  dartsResult.textContent = `🎯 Hit ${zone}! +${reward} TAMA!`;
  triggerAutoSave();
}
...
const moreWalletBtn = document.getElementById('more-wallet-btn');
...
// More Menu Items
if (moreWalletBtn) {
  moreWalletBtn.addEventListener('click', () => {
...
if (moreReferralBtn) {
  moreReferralBtn.addEventListener('click', async () => {
    console.log('🔗 Referral button clicked from More menu!');
    ...
    if (referralModal) {
      ...
      console.log('🔗 Generated:', { code, link });
      ...
      if (codeDisplay) {
        if (window.WALLET_ADDRESS && !window.Telegram?.WebApp?.initDataUnsafe?.user) {
          ...
          console.log('💼 Showing wallet address:', window.WALLET_ADDRESS.substring(0, 12) + '...');
        } else {
          codeDisplay.textContent = code;
          console.log('📱 Showing referral code:', code);
        }
      }
...
if (moreProfileBtn) {
  moreProfileBtn.addEventListener('click', () => {
    ...
    if (userId) {
      ...
      showMessage('🔐 Please connect your wallet or open in Telegram to view profile!', 'error');
    }
    moreMenu.classList.remove('show');
  });
}
...
console.log('Leaderboard elements:', {
  leaderboardBtn,
  leaderboardModal,
  closeLeaderboard,
  leaderboardList,
...
if (saveNameBtn && newNameInput) {
  saveNameBtn.addEventListener('click', async () => {
    ...
    // Show loading
    nameChangeStatus.textContent = '⏳ Saving...';
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('✅ Custom username updated in gameState:', newName);
...
function updatePlayerNameDisplay() {
  if (playerNameDisplay) {
    ...
    const displayName =
      gameState.customUsername ||
      window.Telegram?.WebApp?.initDataUnsafe?.user?.username ||
      window.Telegram?.WebApp?.initDataUnsafe?.user?.first_name ||
      'Player';
    playerNameDisplay.textContent = displayName;
    console.log(
      '✅ Updated player name display:',
      displayName,
      '(custom:',
      !!gameState.customUsername,
      ')'
...
// Call on init
// ⚡ Update player name immediately (no delay needed)
updatePlayerNameDisplay();
...
async function generateReferralCode() {
  console.log('🔑 generateReferralCode() called');

  // ✅ Try multiple sources for Telegram ID
  const telegramUser = window.Telegram?.WebApp?.initDataUnsafe?.user;

  console.log('🔍 Checking Telegram sources:', {
  ...
  if (telegramUser && telegramUser.id) {
    ...
    console.log('✅ Got Telegram ID from WebApp.initDataUnsafe:', telegramId);
  } else if (window.TELEGRAM_USER_ID) {
    // Fallback to global TELEGRAM_USER_ID from auth.js
    telegramId = String(window.TELEGRAM_USER_ID);
    console.log('✅ Using TELEGRAM_USER_ID from auth.js:', telegramId);
  } else {
    console.error('❌ NO TELEGRAM ID FOUND! Both sources are null/undefined');
  }

  if (telegramId) {
    console.log('🔑 Generating referral code for Telegram ID:', telegramId);
  ...
  console.warn('⚠️ No Telegram ID found, returning default code');
  return 'TAMA000000';
}
...
async function updateReferralProgress() {
  try {
    ...
    const [referralsResponse, pendingResponse] = await Promise.all([
      fetch(
        `${SUPABASE_URL}/rest/v1/referrals?referrer_telegram_id=eq.${telegramUser.id}&select=count`,
        {
          headers: {
            apikey: SUPABASE_ANON_KEY,
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      fetch(
        `${SUPABASE_URL}/rest/v1/pending_referrals?referrer_telegram_id=eq.${telegramUser.id}&status=eq.pending&select=count`,
        {
          headers: {
            apikey: SUPABASE_ANON_KEY,
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (statusText) {
      if (referralCount >= 100) {
        statusText.textContent = `🎉 Maximum level reached! (${referralCount} referrals)`;
      } else {
    ...
    milestoneElements.forEach((element, index) => {
      ...
      if (referralCount >= milestones[index]) {
        ...
        referralCount < milestones[index] &&
        (index === 0 || referralCount >= milestones[index - 1])
      ) {
...
${link}`;
    ...
    if (typeof showNotification === 'function') {
      showNotification('🎉 +100 TAMA for sharing!');
    }
    ...
    try {
      ...
      setTimeout(() => {
        shareReferralBtn.textContent = '📤 Share Link';
      }, 2000);
    } catch (err) {
...
async function fetchUserRankUnified(telegramId, walletAddress, userId) {
  try {
    ...
    if (telegramId) {
      ...
      console.warn('⚠️ No user identifier provided');
      return null;
    }

    console.log('🔍 Fetching unified rank:', { telegramId, walletAddress, userId });
    ...
    const result = await response.json();
    console.log('📊 Unified rank result:', result);

    if (result.success && result.found && result.data) {
      return result.data.rank;
    } else {
      console.warn('⚠️ User not found in unified leaderboard');
      return null;
    }
  } catch (error) {
...
async function fetchUserRank(telegramId) {
  try {
    // ⚠️ CRITICAL: Ensure telegramId is always a string
    const telegramIdStr = String(telegramId);
    console.log('🔍 Fetching rank for user:', telegramIdStr);

    // Get user's TAMA balance first
    const userResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/leaderboard?telegram_id=eq.${telegramIdStr}&select=tama`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    console.log('💰 User TAMA:', userTama);

    // Count users with more TAMA (this is the rank)
    const rankResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/leaderboard?select=count&tama=gt.${userTama}`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (!rankResponse.ok) {
      ...
      console.log('⚠️ Count query failed, using fallback method');
      const allUsersResponse = await fetch(
        `${SUPABASE_URL}/rest/v1/leaderboard?select=telegram_id,tama&order=tama.desc&limit=1000`,
        {
          headers: {
            apikey: SUPABASE_ANON_KEY,
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      if (allUsersResponse.ok) {
        ...
        if (userIndex !== -1) {
          ...
          console.log('✅ Rank calculated (fallback):', rank);
          return rank;
        }
      }
      return null;
...
async function loadLeaderboard() {
  try {
    ...
    console.log('📊 Loading UNIFIED leaderboard for user:', currentUserId);
    console.log('📊 Telegram ID:', currentTelegramId, 'Wallet:', currentWalletAddress);
    ...
    const leaderboardData = data.map((player) => {
      ...
      if (player.username && player.username !== 'Player') {
        ...
        displayName =
          player.wallet_address.substring(0, 4) +
          '...' +
          player.wallet_address.substring(player.wallet_address.length - 4);
      }
    ...
    if (currentUserId && rankDisplay) {
      console.log('🔍 Looking for user rank:', {
        currentUserId,
        currentTelegramId,
      ...
      const currentUserData = leaderboardData.find((p) => {
        const matches =
          (currentTelegramId &&
            p.telegram_id &&
            String(p.telegram_id) === String(currentTelegramId)) ||
          (currentWalletAddress &&
            p.wallet_address &&
            String(p.wallet_address) === String(currentWalletAddress)) ||
      ...
      if (currentUserData) {
        ...
        console.log('✅ Rank updated:', currentUserData.rank);
      } else {
        // User not in top 100, fetch their rank separately from unified API
        console.log('⚠️ User not found in top 100, fetching rank separately...');
        fetchUserRankUnified(currentTelegramId, currentWalletAddress, currentUserId)
          .then((rank) => {
            if (rank && rankDisplay) {
              rankDisplay.textContent = `#${rank}`;
              console.log('✅ Rank fetched separately:', rank);
            } else if (rankDisplay) {
              rankDisplay.textContent = '#---';
            }
          })
          .catch((err) => {
            console.warn('⚠️ Failed to fetch user rank:', err);
            if (rankDisplay) {
              rankDisplay.textContent = '#---';
            }
          });
      ...
      if (!currentUserId) {
        console.warn('⚠️ No user ID available for rank display');
      }
    }
    ...
    console.log('⚠️ Falling back to Telegram-only leaderboard...');

    // Fallback to old Telegram-only leaderboard
    try {
      const fallbackResponse = await fetch(
        `${SUPABASE_URL}/rest/v1/leaderboard?select=*&order=tama.desc&limit=50`,
        {
          headers: {
            apikey: SUPABASE_ANON_KEY,
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      if (fallbackResponse.ok) {
        ...
        console.log('✅ Loaded Telegram-only leaderboard (fallback)');
        renderLeaderboard(leaderboardData, currentUserId);

        // Update rank if needed
        if (currentUserId && rankDisplay) {
          const userInList = leaderboardData.find(
            (p) => String(p.telegram_id) === String(currentTelegramId)
          );
          if (userInList) {
...
function renderLeaderboard(data, currentUserId) {
  ...
  data.forEach((player) => {
    ...
    const rankClass =
      player.rank === 1 ? 'gold' : player.rank === 2 ? 'silver' : player.rank === 3 ? 'bronze' : '';
    const rankEmoji =
      player.rank === 1
        ? '🥇'
        : player.rank === 2
    ...
    html += `
                    <div class="leaderboard-item ${isCurrentUser ? 'me' : ''}">
                        ...
                        <div class="leaderboard-player">
                            <div class="leaderboard-name">${player.name}${badge}${
      isCurrentUser ? ' (You)' : ''
    }</div>
                            <div class="leaderboard-level">Level ${player.level}</div>
                        </div>
                        <div class="leaderboard-tama">${formatNumber(player.tama)} 💰</div>
                    </div>
                `;
  });
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        console.log('✅ Loaded economy config from API:', dbConfig.config_name, gameConfig);

        return gameConfig;
      } else {
        console.log('⚠️ No active economy config found in database, using defaults');
        return null;
      }
    } else {
      console.warn('⚠️ Failed to load economy config from API, using defaults');
      return null;
    }
  } catch (error) {
    console.warn('⚠️ Error loading economy config:', error.message);
    return null;
  }
}
...
try {
  ...
  economyChannel.onmessage = (event) => {
    if (event.data.type === 'config-update') {
      ...
      console.log('✅ Economy config updated from admin panel!', event.data.config);
      showMessage('⚙️ Economy settings updated!');
    }
  };
} catch (e) {
...
// ⚡ Load economy config in parallel (non-blocking)
// Don't await - let it load in background
loadEconomyConfig().catch((err) => {
  console.warn('Economy config load failed:', err);
});

// 🎨 Load NFT boost for user
async function loadUserNFTBoost() {
  const userId = window.TELEGRAM_USER_ID || window.WALLET_USER_ID;
  if (!userId) {
    console.warn('⚠️ No user ID (Telegram or Wallet), cannot load NFT boost.');
  ...
  try {
    ...
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/user_nfts?telegram_id=eq.${userId}&select=earning_multiplier,is_active,nft_mint_address`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (data && data.length > 0) {
      ...
      // Apply CAP: максимум 10x (балансировка - было 100x)
      ...
      console.log(`✅ Loaded NFT Boost for user ${userId}:`, {
      ...
      if (window.userNFTBoost > 0) {
        const message = isCapped
          ? `🖼️ NFT Boost Active: ${cappedMultiplier.toFixed(
              1
            )}x (${activeCount} NFTs, CAP 100x reached!)`
          : `🖼️ NFT Boost Active: ${cappedMultiplier.toFixed(1)}x from ${activeCount} NFT${
              activeCount !== 1 ? 's' : ''
            }!`;
        showMessage(message);
      }
      ...
      window.userNFTBoost = 0;
      console.log('ℹ️ No active NFTs found for user, no boost applied.');
    }
  } catch (e) {
...
document.addEventListener('DOMContentLoaded', async function () {
  try {
    console.log(
      '%c🎮 GAME VERSION: 2.0.0 🎮',
      'font-size: 20px; font-weight: bold; color: #00ff00; background: #000000; padding: 10px;'
    );
    console.log(
      '%c📅 BUILD: 2025-12-13 | 13 Languages Support! 🌍',
      'font-size: 14px; color: #ffff00;'
    );
    console.log('✅ DOM loaded, initializing game...');

    // 🌍 Initialize language button with current flag
    const langBtn = document.getElementById('lang-btn');
    if (langBtn && window.i18n) {
      const currentLang = window.i18n.getCurrentLanguage();
      const langInfo = window.i18n.SUPPORTED_LANGUAGES[currentLang];
      if (langInfo) {
        langBtn.textContent = langInfo.flag;
        langBtn.title = langInfo.native;
      }
      // Update flag when language changes
      window.addEventListener('languageChanged', (e) => {
        const newLangInfo = window.i18n.SUPPORTED_LANGUAGES[e.detail.language];
        if (newLangInfo) {
          langBtn.textContent = newLangInfo.flag;
          langBtn.title = newLangInfo.native;
        }
      });
      console.log('🌍 Language button initialized:', currentLang);
    }
      ...
      const observer = new MutationObserver(() => {
        if (walletModal.style.display !== 'none') {
          const walletAddress =
            window.WALLET_ADDRESS ||
            (window.WalletAuth &&
              window.WalletAuth.getState &&
              window.WalletAuth.getState().walletAddress);
    ...
    if (petArea) {
      ...
      if (petArea) {
        ...
        console.log('✅ Pet click handler re-initialized');

        // 🛒 Re-initialize shop button after petArea clone
        const shopBtnNew = document.getElementById('shop-btn');
        const shopModalNew = document.getElementById('shop-modal');
        if (shopBtnNew && shopModalNew) {
          shopBtnNew.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('🛒 Shop button clicked!');
            shopModalNew.classList.add('active');
            document.getElementById('shop-items').style.display = 'block';
            document.getElementById('shop-skins').style.display = 'none';
          });
          console.log('✅ Shop button re-initialized after petArea clone');
        }
      }
    }
    ...
    // ⚡ Load NFT boost immediately (no delay needed)
    try {
    ...
    if (window.Telegram && window.Telegram.WebApp) {
      ...
      console.warn('⚠️ Telegram WebApp not available (browser mode)');
    }
    ...
    document.body.innerHTML = `
                    ...
                    ">
                        ...
                        <details style="max-width: 600px; text-align: left; background: rgba(0,0,0,0.3); padding: 15px; border-radius: 10px; margin-top: 20px;">
                            ...
                            <pre style="overflow: auto; margin-top: 10px; font-size: 12px;">${
                              error.message
...
function initPetSelector() {
  ...
  if (!document.body.getAttribute('data-pet')) {
    document.body.setAttribute('data-pet', savedPet);
    console.log('✅ Set initial data-pet:', savedPet);
  }
...
// ⚠️ REMOVED: Old feedPet(), playWithPet(), healPet() functions
...
// ⚠️ REMOVED: Old duplicate handlers (feedPet, playWithPet, healPet)
...
function selectGame(game) {
  // Для luckyslots - открываем slots.html
  if (game === 'luckyslots') {
    const userId = window.TELEGRAM_USER_ID || '';
  ...
  // Для platformer - открываем super-tama-bros.html
  if (game === 'platformer') {
    const userId = window.TELEGRAM_USER_ID || '';
  ...
  // Для colormatch - открываем tama-color-match.html
  if (game === 'colormatch') {
    const userId = window.TELEGRAM_USER_ID || '';
  ...
  // Для shooter - открываем tama-shooter.html
  if (game === 'shooter') {
    const userId = window.TELEGRAM_USER_ID || '';
...
function startCatchPetGame() {
  ...
  if (window.catchPetState.status) {
    window.catchPetState.status.textContent = 'Click on the pet to catch it! 🎯';
  }
...
// Move Pet Randomly
function movePet() {
  if (!window.catchPetState.active || !window.catchPetState.pet || !window.catchPetState.gameArea)
    return;
...
function catchPet(event) {
  ...
  if (distance < 60) {
    ...
    // Update status
    if (window.catchPetState.status) {
      window.catchPetState.status.textContent = `Great catch! +${Math.floor(
        10 * window.catchPetState.speed
      )} points! 🎉`;
    }
  } else {
    // Miss
    if (window.catchPetState.status) {
      window.catchPetState.status.textContent = 'Miss! Try again! 🎯';
    }
  }
}
...
// NFT Modal & Wallet Integration 🖼️
...
function initNFTModal() {
  ...
  if (nftBtn) {
    ...
    console.warn('⚠️ NFT button not found in DOM');
  }
  ...
  if (mintWithSolBtn) {
    mintWithSolBtn.addEventListener('click', async () => {
      ...
      if (isTelegramWebApp) {
        ...
        // Build mint page URL
        // 🔐 SECURITY: Don't pass tama in URL - it will be loaded from DB
        ...
        const phantomDeepLink = `https://phantom.app/ul/v1/connect?app_url=${encodeURIComponent(
          mintUrl
        )}&redirect_link=${encodeURIComponent(mintUrl)}`;
  ...
  async function navigateToMintPage() {
    ...
    try {
      ...
      console.log('✅ Game state saved before NFT mint page');
    } catch (err) {
      console.warn('⚠️ Failed to save before navigation:', err);
    }
    ...
    if (window.solana && window.solana.isConnected) {
      try {
        ...
        if (wallet) {
          ...
          console.log('✅ Wallet address saved:', wallet.toString());
        }
      } catch (err) {
        console.warn('⚠️ Failed to save wallet:', err);
      }
    }
  ...
  function showMintTierSelection() {
    ...
    modal.innerHTML = `
                    <div style="background: linear-gradient(135deg, rgba(29, 53, 87, 0.95), rgba(69, 123, 157, 0.9)); border: 2px solid #8AC926; border-radius: 20px; padding: 30px; max-width: 500px; width: 90%; max-height: 90vh; overflow-y: auto;">
                        <h2 style="color: #FFD700; text-align: center; margin-bottom: 20px;">💎 Mint NFT with SOL</h2>
                        <p style="text-align: center; color: rgba(255,255,255,0.8); margin-bottom: 20px;">Choose NFT tier:</p>
                        <div style="display: flex; flex-direction: column; gap: 10px;">
                            <button class="share-btn" onclick="window.mintNFTWithSOL('Bronze')" style="background: rgba(205, 127, 50, 0.3); border: 2px solid #CD7F32;">🥉 Bronze - 0.15 SOL</button>
                            <button class="share-btn" onclick="window.mintNFTWithSOL('Silver')" style="background: rgba(192, 192, 192, 0.3); border: 2px solid #C0C0C0;">🥈 Silver - ~1 SOL</button>
                            <button class="share-btn" onclick="window.mintNFTWithSOL('Gold')" style="background: rgba(255, 215, 0, 0.3); border: 2px solid #FFD700;">🥇 Gold - ~3 SOL</button>
                            <button class="share-btn" onclick="window.mintNFTWithSOL('Platinum')" style="background: rgba(229, 228, 226, 0.3); border: 2px solid #E5E4E2;">💎 Platinum - ~10 SOL</button>
                            <button class="share-btn" onclick="window.mintNFTWithSOL('Diamond')" style="background: rgba(185, 242, 255, 0.3); border: 2px solid #B9F2FF;">🔷 Diamond - ~50 SOL</button>
                        </div>
...
async function connectPhantomWallet() {
  try {
    // ⚠️ Check if running in Telegram WebApp (Phantom doesn't work there)
    const isTelegramWebApp = window.Telegram && window.Telegram.WebApp;

    if (isTelegramWebApp) {
      // In Telegram WebApp, Phantom doesn't work - need to open in browser
      const confirmed = confirm(
        '⚠️ Phantom Wallet не работает в Telegram WebApp!\n\n' +
          'Для подключения кошелька нужно открыть игру в браузере.\n\n' +
          'Открыть в браузере сейчас?'
      );
    ...
    // 🔗 NEW: Auto-link wallet to Telegram account
    await linkWalletToTelegramAccount(walletAddress);

    // 🔄 Sync account data (balance, NFTs, etc.)
...
// 🔗 Auto-link wallet to Telegram account and merge orphaned NFTs
async function linkWalletToTelegramAccount(walletAddress) {
  const telegramId = window.TELEGRAM_USER_ID;
  if (!telegramId) {
    console.log('⚠️ No Telegram ID found, skipping wallet linking');
    return;
  }

  try {
    console.log('🔗 Linking wallet to Telegram account...', { walletAddress, telegramId });
    ...
    if (leaderboardResponse.ok) {
      console.log('✅ Wallet address saved to leaderboard');
    } else {
      console.warn(
        '⚠️ Failed to save wallet_address to leaderboard:',
    ...
    try {
      const nftsResponse = await fetch(
        `${SUPABASE_URL}/rest/v1/user_nfts?wallet_address=eq.${walletAddress}&telegram_id=is.null&select=id`,
        {
          headers: {
            apikey: SUPABASE_ANON_KEY,
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      if (nftsResponse.ok) {
        ...
        console.log(`✅ Found ${orphanedNftIds.length} orphaned NFT(s) in user_nfts table`);
      }
    } catch (e) {
      console.log(
        '⚠️ wallet_address column might not exist in user_nfts, trying transactions method...'
      );
    }

    // Method 2: Also search transactions table for SOL mints with this wallet (backup method)
    const transactionsResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/transactions?type=eq.nft_mint_sol&select=metadata`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (transactionsResponse.ok) {
      ...
      if (transactionNftIds.length > 0) {
        console.log(
          `✅ Found ${transactionNftIds.length} additional orphaned NFT(s) from transactions`
        );
      }
    }

    if (orphanedNftIds.length === 0) {
      console.log('✅ No orphaned NFTs found for this wallet');
      return;
    }

    console.log(`🔍 Found ${orphanedNftIds.length} orphaned NFT(s) to link:`, orphanedNftIds);
    ...
    for (const nftId of orphanedNftIds) {
      ...
      if (updateResponse.ok) {
        console.log(`✅ Linked NFT #${nftId} to Telegram account`);
      } else {
        console.warn(`⚠️ Failed to link NFT #${nftId}:`, await updateResponse.text());
      }
    }

    // Step 4: Show success notification
    if (orphanedNftIds.length > 0) {
      showMessage(`🎉 Linked ${orphanedNftIds.length} NFT(s) to your Telegram account!`);
      console.log(`✅ Successfully linked ${orphanedNftIds.length} orphaned NFT(s)`);
    }
  } catch (err) {
...
async function syncAccountData() {
  ...
  if (!telegramId || !wallet) {
    console.log('⚠️ Cannot sync: missing telegram_id or wallet_address');
    return;
  }

  try {
    console.log('🔄 Syncing account data...', { telegramId, wallet });
    ...
    if (balanceResponse.ok) {
      ...
      if (balanceData && balanceData.length > 0 && balanceData[0].tama !== undefined) {
        ...
        if (dbBalance > gameState.tama) {
          ...
          console.log('✅ TAMA balance synced from database:', dbBalance);
        }
      }
    }
  ...
  try {
    ...
    transaction.add(
      SystemProgram.transfer({
        ...
        lamports: Math.floor(priceSOL * 1e9 * 0.5),
      })
    );
    ...
    transaction.add(
      SystemProgram.transfer({
        ...
        lamports: Math.floor(priceSOL * 1e9 * 0.3),
      })
    );
    ...
    transaction.add(
      SystemProgram.transfer({
        ...
        lamports: Math.floor(priceSOL * 1e9 * 0.2),
      })
    );
    ...
    console.log('💰 Creating SOL distribution transaction...');
    console.log(`  🏦 Treasury Main: ${(priceSOL * 0.5).toFixed(6)} SOL (50%)`);
    console.log(`  💧 Treasury Liquidity: ${(priceSOL * 0.3).toFixed(6)} SOL (30%)`);
    console.log(`  👥 Treasury Team: ${(priceSOL * 0.2).toFixed(6)} SOL (20%)`);
    ...
    console.log('✅ SOL distribution transaction confirmed:', signature);
    return signature;
  } catch (err) {
...
window.mintNFTWithSOL = async function (tierName) {
  try {
    console.log(`💎 Minting ${tierName} NFT with SOL in-game...`);
    ...
    const priceResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/nft_bonding_state?tier_name=eq.${tierName}&select=current_price`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    const confirmed = confirm(
      `Mint ${tierName} NFT for ${price} SOL ($${priceUSD})?\n\n` +
        `You will get ×${boostMap[tierName]} earning boost!\n\n` +
        `Continue?`
    );
    ...
    try {
      ...
      console.log('✅ Distribution transaction completed:', transactionSignature);
    } catch (distError) {
      console.error('⚠️ Distribution transaction failed:', distError);
    ...
    if (result.success) {
      showMessage(
        `✅ ${tierName} NFT #${result.design_number || result.nft_id || 'N/A'} minted!`,
        'success'
      );
...
function getNFTImageUrl(tier, rarity) {
  ...
  const ipfsUrls = {
    bronze: {
      ...
      uncommon:
        'https://gateway.lighthouse.storage/ipfs/bafkreibnoiown4k6dyhxvv642ep6av6xwkgtqvusrhhn7l4janrgfjixbq',
    ...
    silver: {
      uncommon:
        'https://gateway.lighthouse.storage/ipfs/bafkreibp7zxf6fqilehacookucnyhzbqkvaqqbuk3jel7irsa2dzzvnw2a',
      ...
      legendary:
        'https://gateway.lighthouse.storage/ipfs/bafkreigywjdjw3vxopv4blicqioyx5fyqpwcvs22s2ea377rofvh2sslnm',
    },
    gold: {
      common:
        'https://gateway.lighthouse.storage/ipfs/bafkreicywzvyse3immuhakmd4dvv22gxsikmzhn4q7cjkmzjpp7253ftse',
      uncommon:
        'https://gateway.lighthouse.storage/ipfs/bafkreibp7zxf6fqilehacookucnyhzbqkvaqqbuk3jel7irsa2dzzvnw2a',
      rare: 'https://gateway.lighthouse.storage/ipfs/bafkreidnwtfwftmcsexgmf6p5qn5jorgwmtl4w2jegyyo7gnynvq2qe334',
      epic: 'https://gateway.lighthouse.storage/ipfs/bafkreifkxigyyudtynmn4ffmt2gx7getqs3jfzy2nqdjrzaplpelf3tozq',
    ...
    platinum: {
      ...
      legendary:
        'https://gateway.lighthouse.storage/ipfs/bafkreihrwin3ld34uner7rpwggke2pfnn5beb3eyrw7vrjmw6n5hen5hie',
    },
    diamond: {
      ...
      legendary:
        'https://gateway.lighthouse.storage/ipfs/bafkreidtqr62aeflchsghhdoo4m7tv33j7za5w3ttzqziwkl4u4cmgz7tq',
    },
  };
...
async function loadNFTCollection() {
  console.log('🔍 loadNFTCollection called');
  try {
    const userId = window.TELEGRAM_USER_ID;
    console.log('👤 Current user ID:', userId);
    if (!userId) {
      console.warn('⚠️ No user ID for NFT loading');
    ...
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/user_nfts?telegram_id=eq.${userId}&order=id.desc&select=*,nft_designs(image_url,design_theme,design_variant)`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (!response.ok) {
      ...
      console.warn('⚠️ Join query failed, trying simple query...');
      const simpleResponse = await fetch(
        `${SUPABASE_URL}/rest/v1/user_nfts?telegram_id=eq.${userId}&order=id.desc&select=*`,
        {
          headers: {
            apikey: SUPABASE_ANON_KEY,
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    const nftsWithDesignIds = nfts.filter(
      (n) => n.nft_design_id && (!n.nft_designs || !n.nft_designs.image_url)
    );
    if (nftsWithDesignIds.length > 0) {
      console.log('🖼️ Fetching images for', nftsWithDesignIds.length, 'NFTs...');
      const designIds = [...new Set(nftsWithDesignIds.map((n) => n.nft_design_id))];

      try {
        const designsResponse = await fetch(
          `${SUPABASE_URL}/rest/v1/nft_designs?id=in.(${designIds.join(
            ','
          )})&select=id,image_url,design_theme,design_variant`,
          {
            headers: {
              apikey: SUPABASE_ANON_KEY,
              Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
        ...
        console.warn('⚠️ Failed to fetch design images:', e);
      }
    }
    ...
    if (nfts && nfts.length > 0) {
      ...
      const totalBoost = activeNFTsList.reduce(
        (sum, n) => sum + (parseFloat(n.earning_multiplier) || 0),
        0
      );

      nftStats.innerHTML = `
                        <div style="display: flex; justify-content: space-around; flex-wrap: wrap; gap: 15px;">
                            <div><strong>Total:</strong> ${totalNFTs} NFT${
        totalNFTs !== 1 ? 's' : ''
      }</div>
      ...
      nftGrid.innerHTML = nfts
        .map((nft) => {
          ...
          const svgContent = `<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200"><text x="50%" y="50%" font-size="100" text-anchor="middle" dy=".3em">${
            tierEmoji[tier] || '🎮'
          }</text></svg>`;
          ...
          return `
                            <div class="nft-card" style="border-color: ${tierColor};" data-nft='${nftData}' onclick="showNFTDetail(${
            nft.id
          })">
                                ...
                                <div style="font-size: 16px; color: #8AC926; margin-bottom: 8px; font-weight: bold;">⚡ ${multiplier}x Boost</div>
                                ${
                                  isActive
                                    ? '<div style="font-size: 12px; color: #10b981; font-weight: bold;">✅ Active</div>'
                                    : '<div style="font-size: 12px; color: #ef4444;">❌ Inactive</div>'
                                }
                            </div>
                        `;
        })
    ...
    if (nftGrid)
      nftGrid.innerHTML =
        '<p style="text-align: center; color: #ef4444;">Failed to load NFTs. Please try again.</p>';
  }
}
...
async function showNFTDetail(nftId) {
  try {
    ...
    // Загружаем информацию о транзакции минта
    ...
    try {
      ...
      if (userId) {
        const txResponse = await fetch(
          `${SUPABASE_URL}/rest/v1/transactions?user_id=eq.${userId}&type=eq.nft_mint&metadata->>nft_id=eq.${nftId}&order=created_at.desc&limit=1`,
          {
            headers: {
              apikey: SUPABASE_ANON_KEY,
              Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
        ...
        if (txResponse.ok) {
          ...
          if (transactions && transactions.length > 0) {
            ...
            const metadata =
              typeof mintTransaction.metadata === 'string'
                ? JSON.parse(mintTransaction.metadata)
                : mintTransaction.metadata;

            // Определяем причину минта
            if (metadata.purchase_method === 'tama') {
              mintReason = `Purchased with ${
                metadata.cost_tama || nft.purchase_price_tama || 0
              } TAMA`;
    ...
    const detailHTML = `
                    <div class="nft-detail-header">
                        ...
                        <div class="nft-detail-info">
                            ...
                            <div style="margin-bottom: 15px;">
                                ...
                                ${
                                  isOnChain
                                    ? '<span class="nft-detail-badge" style="background: #8b5cf6; color: #fff;">🔗 On-Chain</span>'
                                    : ''
                                }
                            </div>
                            <div class="nft-detail-stats">
                                <div class="nft-detail-stat">
                                    <div class="nft-detail-stat-label">⚡ Earning Boost</div>
                                    <div style="font-size: 24px; font-weight: bold; color: #8AC926;">${multiplier}x</div>
                                </div>
                                <div class="nft-detail-stat">
                                    <div class="nft-detail-stat-label">💰 Purchase Price</div>
                                    <div style="font-size: 18px; font-weight: bold;">
                                        ${
                                          nft.purchase_price_tama
                                            ? `${nft.purchase_price_tama} TAMA`
                                            : ''
                                        }
                                        ${
                                          nft.purchase_price_sol
                                            ? `${nft.purchase_price_sol} SOL`
                                            : ''
                                        }
                                        ${
                                          !nft.purchase_price_tama && !nft.purchase_price_sol
                                            ? 'Free'
                                            : ''
                                        }
                                    </div>
                                ...
                                <div class="nft-detail-stat">
                                    <div class="nft-detail-stat-label">📅 Minted Date</div>
                                ...
                                ${
                                  explorerLink
                                    ? `
                                <div class="nft-detail-stat">
                                    <div class="nft-detail-stat-label">🔗 Blockchain</div>
                    ...
                    <div style="margin-top: 30px; padding: 20px; background: rgba(255,255,255,0.05); border-radius: 10px; border: 1px solid rgba(138, 201, 38, 0.2);">
                        <h3 style="margin: 0 0 15px 0; color: #8AC926; font-size: 20px;">📝 Mint Information</h3>
                            ...
                            ${
                              mintTransaction && mintTransaction.metadata
                                ...
                                <div style="margin-top: 10px; padding: 10px; background: rgba(0,0,0,0.3); border-radius: 5px; font-size: 12px; font-family: monospace;">
                                    ...
                                    ${
                                      mintTransaction.metadata.transaction_signature
                                        ? `<strong>Signature:</strong> ${mintTransaction.metadata.transaction_signature.substring(
                                            0,
                                            20
                                          )}...`
                                        : ''
...
function updateSavedWalletsUI() {
  ...
  savedWallets.forEach((wallet, index) => {
    ...
    option.value = index;
    option.textContent = `${wallet.name || 'Wallet'} - ${wallet.address.substring(
      0,
      8
...
function initWithdrawModal() {
  ...
  if (withdrawBtn && withdrawModal) {
    withdrawBtn.addEventListener('click', async () => {
      ...
      await loadSavedWalletAddress(); // Загружаем сохраненный адрес
      await loadWithdrawHistory();
      loadSavedWallets(); // Загружаем список сохраненных адресов
...
async function loadSavedWalletAddress() {
  try {
    ...
    // Загрузить из базы данных (leaderboard)
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/leaderboard?telegram_id=eq.${userId}&select=wallet_address`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
...
async function loadWithdrawHistory() {
  try {
    ...
    // API возвращает { success: true, withdrawals: [...] }
    const withdrawals = result.withdrawals || result.data || [];

    if (!Array.isArray(withdrawals)) {
      console.error('Invalid withdrawals format:', withdrawals);
      historyDiv.innerHTML =
        '<div style="text-align: center; padding: 20px; color: rgba(255,255,255,0.6);">Invalid response format</div>';
      return;
    }
    ...
    historyDiv.innerHTML = withdrawals
      .map((w, index) => {
        ...
        // API возвращает amount_sent (уже с вычетом комиссии) или amount - fee
        ...
        const statusColor =
          status === 'completed' ? '#10b981' : status === 'pending' ? '#fbbf24' : '#ef4444';
        ...
        return `
                        <div style="padding: 20px; margin-bottom: 15px; background: linear-gradient(135deg, rgba(255,255,255,0.08) 0%, rgba(255,255,255,0.03) 100%); border-radius: 15px; border: 2px solid rgba(138, 201, 38, 0.3); box-shadow: 0 4px 15px rgba(0,0,0,0.2); transition: all 0.3s;">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px;">
                                <div>
                                    <div style="font-weight: bold; color: #8AC926; font-size: 24px; margin-bottom: 5px;">${formatNumber(
                                      amount
                                    )} TAMA</div>
                                    <div style="font-size: 14px; color: rgba(255,255,255,0.6);">
                                        Fee: ${formatNumber(fee)} TAMA (Total: ${formatNumber(
          totalAmount
        )} TAMA)
                                    </div>
                                </div>
          ...
          status.charAt(0).toUpperCase() + status.slice(1)
                            ...
                            <div style="padding: 12px; background: rgba(0,0,0,0.2); border-radius: 10px; margin-bottom: 10px;">
                                <div style="font-size: 12px; color: rgba(255,255,255,0.7); margin-bottom: 5px;">📅 Date:</div>
                            ...
                            ${
                              wallet && wallet !== 'N/A'
                                ? `
                                <div style="padding: 12px; background: rgba(0,0,0,0.2); border-radius: 10px; margin-bottom: 10px;">
                                    <div style="font-size: 12px; color: rgba(255,255,255,0.7); margin-bottom: 5px;">👛 Wallet:</div>
                            ...
                            ${
                              signature
                                ...
                                <div style="padding: 12px; background: rgba(138, 201, 38, 0.1); border-radius: 10px; border: 1px solid rgba(138, 201, 38, 0.3);">
                                    <div style="font-size: 12px; color: rgba(255,255,255,0.7); margin-bottom: 5px;">🔗 Transaction:</div>
                                    <a href="https://solscan.io/tx/${signature}?cluster=devnet" target="_blank"
                                       ...
                                       onmouseout="this.style.background='rgba(138, 201, 38, 0.1)'; this.style.transform='translateX(0)';">
                                        ${signature.substring(0, 20)}...${signature.substring(
                                    signature.length - 8
                                  )}
    ...
    if (historyDiv) {
      historyDiv.innerHTML =
        '<div style="text-align: center; padding: 20px; color: rgba(255,255,255,0.6);">Failed to load history</div>';
    }
  }
}
...
function updateWithdrawSummary() {
  ...
  const wallet =
    withdrawWalletAddress || document.getElementById('withdraw-wallet-input').value.trim();
  ...
  if (amount > balance) {
    ...
    summaryDiv.innerHTML = `
                    ...
                    <div style="margin-top: 5px; font-size: 14px;">You have: ${formatNumber(
                      balance
                    )} TAMA</div>
  ...
  try {
    ...
    if (result.success) {
      ...
      statusDiv.innerHTML = `
                        ...
                        <div style="font-size: 16px; margin-bottom: 10px;">💰 Sent: <strong>${formatNumber(
                          netAmount
                        )} TAMA</strong></div>
      ...
      // Сохранить адрес кошелька для будущих выводов
      if (wallet && wallet.length >= 32) {
        localStorage.setItem('withdraw_wallet_address', wallet);
        // Также сохранить в базу данных
        try {
          const saveResponse = await fetch(
            `${SUPABASE_URL}/rest/v1/leaderboard?telegram_id=eq.${userId}`,
            {
              method: 'PATCH',
              headers: {
                apikey: SUPABASE_ANON_KEY,
      ...
      // Reset form after 5 seconds (но сохраняем адрес)
      setTimeout(() => {
        document.getElementById('withdraw-amount-input').value = '';
        // НЕ очищаем адрес - он сохранен
        withdrawPhantomConnected = false;
        document.getElementById('phantom-connect-section').style.display = 'block';
        document.getElementById('phantom-connected').style.display = 'none';
...
async function mintNewNFT() {
  try {
    ...
    if (response.ok) {
      alert(
        `🎉 NFT Minted! ${getNFTEmoji(randomPet)} ${randomPet.toUpperCase()} - ${randomRarity}`
      );
      await loadNFTCollection();
    } else {
