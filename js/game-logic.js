        ...
        function clickPet(e) {
            ...
            if (window.userNFTBoost && window.userNFTBoost > 0) {
                ...
                if (gameState.combo === 1 || gameState.combo === 0) {
                    console.log(`🖼️ NFT Boost Applied: ${earnedTamaBeforeBoost.toFixed(2)} TAMA × ${multiplier.toFixed(2)} = ${earnedTama.toFixed(2)} TAMA`);
                }
            }
            ...
            if (availableToday <= 0) {
                ...
                console.log('🚫 Daily limit reached! Earned today:', gameState.dailyEarned, '/ Limit:', gameState.personalDailyLimit);
            ...
            if (earnedTama > availableToday) {
                console.log(`⚠️ Capping reward: ${earnedTama.toFixed(2)} → ${availableToday.toFixed(2)} TAMA (daily limit)`);
        ...
        try {
            ...
            shopBtns.forEach((el, idx) => { if (idx > 0) el.remove(); });
        } catch (e) {
            // ignore
        }
        ...
        function loadOwnedSkinsShop() {
            ...
            ownedSkins.forEach(skinId => {
                ...
                const emojiMap = {
                    'kawai': '🌸', 'retro': '🕹️', 'neko': '🐱', 'cyber': '🤖',
                    'premium': '⭐', 'alien': '👽', 'kitsune': '🦊',
                    'santa': '🎅', 'snowman': '⛄', 'reindeer': '🦌', 'tree': '🎄'
                };
                ...
                skinCard.innerHTML = `
                    ...
                    <div class="skin-preview" style="font-size: 80px; margin: 20px 0;">${emojiMap[skin.id] || '🎨'}</div>
        ...
        function openPetSelector() {
            ...
            Object.keys(PET_TYPES).forEach(petKey => {
                ...
                petCard.innerHTML = `
                    ...
                    <div style="font-size: 12px; color: #fbbf24;">${isOwned ? (isSelected ? '✓ Selected' : 'Select') : `${pet.price} TAMA`}</div>
                `;
            ...
            try {
                ...
                if (!cloudLoaded && (forceAuth === 'telegram' || (!forceAuth && hasTelegramUser))) {
                    ...
                    try {
                        ...
                        const devEnabled = isLocalhost && (new URLSearchParams(window.location.search).get('dev') === '1');
                        const forcedUserId = devEnabled && window.TELEGRAM_USER_ID ? String(window.TELEGRAM_USER_ID) : '';
                    ...
                    if (!window.WALLET_ADDRESS && window.WalletAuth) {
                        ...
                        console.warn('⚠️ No authentication method available - game will work with localStorage only');
                    }
                }
        ...
        async function loadFromSupabase(userId) {
            try {
                ...
                const userIdStr = String(userId);
                console.log('🔍 Loading from Supabase with userId:', userIdStr, '(type:', typeof userIdStr, ')');
        ...
        async function logTransaction(type, amount, balanceBefore, balanceAfter, metadata = {}) {
            ...
            const supabaseUrl = window.CONFIG?.SUPABASE_URL || typeof SUPABASE_URL !== 'undefined' ? SUPABASE_URL : 'https://zfrazyupameidxpjihrh.supabase.co';
            const supabaseKey = window.CONFIG?.SUPABASE_ANON_KEY || typeof SUPABASE_ANON_KEY !== 'undefined' ? SUPABASE_ANON_KEY : null;
        ...
        function triggerAutoSave(forceCritical = false) {
            ...
            if (window.WALLET_ADDRESS && window.WalletSave) {
                console.log('💾 Saving via wallet:', { address: window.WALLET_ADDRESS, userId: window.WALLET_USER_ID });
        ...
        document.querySelectorAll('.game-card').forEach(card => {
            card.addEventListener('click', (e) => {
                ...
                const arena = document.getElementById(`${gameName}-arena`) || document.getElementById(`${gameName.replace('lucky', '')}-arena`);
                if (arena) {
        ...
        function initCareRitual() {
            ...
            function doCare(action) {
                ...
                if (action === 'sleep') {
                    ...
                    updateUI();
                    setCareScene(atNight ? 'You tuck him in. <strong>Soft breathing…</strong>' : 'A short nap. <strong>He calms down.</strong>', '😵', 2400);
                    triggerAutoSave();
                    return;
                }
        ...
        try { initCareRitual(); } catch (e) { console.warn('Care ritual init failed', e); }
            ...
            function animateWheel() {
                ...
                if (progress < 1) {
                    ...
                    if (multiplier >= 5) {
                        ...
                        wheelResult.textContent = `😔 ${multiplier}x = ${reward} TAMA (lost ${wheelState.betAmount - reward})`;
        ...
        document.getElementById('tower-add-btn')?.addEventListener('click', () => {
            ...
            const resultDiv = document.getElementById('tower-result');
            resultDiv.textContent = `🏗️ Tower: ${towerState.blocks} blocks | Fall chance: ${(towerState.fallChance * 100).toFixed(1)}%`;
            resultDiv.style.color = '#fff';
        });
        ...
        function updatePlayerNameDisplay() {
            if (playerNameDisplay) {
                ...
                console.log('✅ Updated player name display:', displayName, '(custom:', !!gameState.customUsername, ')');
            }
        }
...
${link}`;
                ...
                if (data && data.length > 0) {
                    ...
                    if (window.userNFTBoost > 0) {
                        const message = isCapped
                            ? `🖼️ NFT Boost Active: ${cappedMultiplier.toFixed(1)}x (${activeCount} NFTs, CAP 100x reached!)`
                            : `🖼️ NFT Boost Active: ${cappedMultiplier.toFixed(1)}x from ${activeCount} NFT${activeCount !== 1 ? 's' : ''}!`;
                        showMessage(message);
                    }
                } else {
                    window.userNFTBoost = 0;
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            try {
                console.log('%c🎮 GAME VERSION: 2.0.0 🎮', 'font-size: 20px; font-weight: bold; color: #00ff00; background: #000000; padding: 10px;');
                console.log('%c📅 BUILD: 2025-12-13 | 13 Languages Support! 🌍', 'font-size: 14px; color: #ffff00;');
        ...
        function catchPet(event) {
            ...
            if (distance < 60) { // Catch radius
                ...
                // Update status
                if (window.catchPetState.status) {
                    window.catchPetState.status.textContent = `Great catch! +${Math.floor(10 * window.catchPetState.speed)} points! 🎉`;
                }
            } else {
                // Miss
                if (window.catchPetState.status) {
        ...
        async function linkWalletToTelegramAccount(walletAddress) {
            ...
            try {
                ...
                if (leaderboardResponse.ok) {
                    ...
                    console.warn('⚠️ Failed to save wallet_address to leaderboard:', await leaderboardResponse.text());
                }
                ...
                try {
                    ...
                    console.log('⚠️ wallet_address column might not exist in user_nfts, trying transactions method...');
                }
                ...
                if (transactionsResponse.ok) {
                    ...
                    if (transactionNftIds.length > 0) {
                        console.log(`✅ Found ${transactionNftIds.length} additional orphaned NFT(s) from transactions`);
                    }
                }
        ...
        async function createAndSendDistributionTransaction(priceSOL) {
            try {
                ...
                console.log(`  🏦 Treasury Main: ${(priceSOL * 0.50).toFixed(6)} SOL (50%)`);
                console.log(`  💧 Treasury Liquidity: ${(priceSOL * 0.30).toFixed(6)} SOL (30%)`);
                console.log(`  👥 Treasury Team: ${(priceSOL * 0.20).toFixed(6)} SOL (20%)`);
                    ...
                    <div class="nft-detail-header">
                        ...
                        <div class="nft-detail-info">
                            ...
                            <div style="margin-bottom: 15px;">
                                ...
                                ${isOnChain ? '<span class="nft-detail-badge" style="background: #8b5cf6; color: #fff;">🔗 On-Chain</span>' : ''}
                            </div>
            ...
            try {
                ...
                if (result.success) {
                    ...
                    statusDiv.innerHTML = `
                        ...
                        <div style="font-size: 16px; margin-bottom: 10px;">💰 Sent: <strong>${formatNumber(netAmount)} TAMA</strong></div>
        ...
        async function mintNewNFT() {
            try {
                ...
                if (response.ok) {
                    alert(`🎉 NFT Minted! ${getNFTEmoji(randomPet)} ${randomPet.toUpperCase()} - ${randomRarity}`);
                    await loadNFTCollection();
                } else {
